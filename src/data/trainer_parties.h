static const struct TrainerMonItemCustomMoves sParty_Sawyer1[] = {
    {
    .iv = 100,
    .evs = {0, 252, 252, 0, 6, 0},
    .lvl = 38,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_HARD_STONE,
    .moves = {MOVE_ROCK_SLIDE, MOVE_EXPLOSION, MOVE_TAKE_DOWN, MOVE_ROCK_BLAST},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .evs = {0, 252, 252, 0, 6, 0},
    .lvl = 38,
    .species = SPECIES_RHYDON,
    .heldItem = ITEM_SOFT_SAND,
    .moves = {MOVE_ROCK_SLIDE, MOVE_DRILL_RUN, MOVE_TAKE_DOWN, MOVE_BULLDOZE},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .evs = {0, 0, 6, 252, 252, 0},
    .lvl = 38,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_ANCIENT_POWER, MOVE_COSMIC_POWER, MOVE_RAPID_SPIN},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout1[] = {
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_POISON_BARB,
    .moves = {MOVE_SURF, MOVE_SLUDGE_BOMB, MOVE_GIGA_DRAIN, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_MACH_PUNCH, MOVE_ICE_PUNCH, MOVE_FAKE_OUT, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_QWILFISH,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_LIQUIDATION, MOVE_POISON_JAB, MOVE_ICE_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_URSARING,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_BODY_SLAM, MOVE_SHADOW_CLAW, MOVE_AERIAL_ACE, MOVE_SEED_BOMB},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_VILEPLUME,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_MOONBLAST, MOVE_TOXIC},
    .abilityNums = 1
    },
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout2[] = {
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_ICE_BEAM, MOVE_EARTH_POWER, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_GOLDUCK,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_MUDDY_WATER, MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_HYPNOSIS},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_THUNDERBOLT, MOVE_ICE_FANG, MOVE_FLAMETHROWER},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout3[] = {
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_GASTRODON_E,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTH_POWER, MOVE_SURF, MOVE_ICE_BEAM, MOVE_CURSE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_PURUGLY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_BODY_SLAM, MOVE_SHADOW_CLAW, MOVE_FAKE_OUT, MOVE_QUICK_ATTACK},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_HUNTAIL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_TAIL, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_AQUA_JET},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_DARK_PULSE, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GUNK_SHOT, MOVE_ROCK_SLIDE, MOVE_DARK_PULSE, MOVE_FLAMETHROWER},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout4[] = {
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CRABHAMMER, MOVE_KNOCK_OFF, MOVE_METAL_CLAW, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_RELICANTH,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_WATERFALL, MOVE_ICE_BEAM, MOVE_ANCIENT_POWER},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_YANMEGA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BUG_BUZZ, MOVE_AIR_SLASH, MOVE_ANCIENT_POWER, MOVE_GIGA_DRAIN},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_WEAVILE,
    .heldItem = ITEM_NEVER_MELT_ICE,
    .moves = {MOVE_ICE_SHARD, MOVE_ICE_PUNCH, MOVE_NIGHT_SLASH, MOVE_BRICK_BREAK},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_GruntSeafloorCavern1[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SKUNTANK,
    .heldItem = ITEM_POISON_BARB,
    .moves = {MOVE_GUNK_SHOT, MOVE_NIGHT_SLASH, MOVE_EXPLOSION, MOVE_FLAMETHROWER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_EXEGGUTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE, MOVE_SLUDGE_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_TAIL, MOVE_ICE_FANG, MOVE_EARTHQUAKE, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_AGGRON,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntSeafloorCavern2[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SHARPEDO,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_LIQUIDATION, MOVE_CRUNCH, MOVE_POISON_JAB, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_SEED_BOMB, MOVE_MACH_PUNCH, MOVE_ROCK_SLIDE, MOVE_SPORE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EARTHQUAKE, MOVE_SURF, MOVE_ICE_BEAM, MOVE_AMNESIA},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_THUNDERBOLT, MOVE_EXPLOSION, MOVE_THUNDER_WAVE, MOVE_MIRROR_COAT},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntSeafloorCavern3[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_ICE_BEAM, MOVE_EARTH_POWER, MOVE_AMNESIA},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_SHADOW_BALL, MOVE_ZEN_HEADBUTT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_FIRE_FANG},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_Gabrielle1[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_MOONBLAST, MOVE_SHADOW_BALL, MOVE_CALM_MIND},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_SABLEYE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BRICK_BREAK, MOVE_SHADOW_SNEAK, MOVE_POWER_GEM, MOVE_GIGA_DRAIN},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SEISMIC_TOSS, MOVE_SHADOW_BALL, MOVE_HYPER_VOICE, MOVE_TOXIC},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntPetalburgWoods[] = {
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_SNORUNT,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_BITE, MOVE_ICE_SHARD, MOVE_HEADBUTT, MOVE_ASTONISH},
    .abilityNums = 1
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_TANGELA,
    .heldItem = ITEM_APICOT_BERRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_LEECH_SEED, MOVE_ACID, MOVE_STUN_SPORE},
    .abilityNums = 1
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_CARVANHA,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_BITE, MOVE_AQUA_JET, MOVE_POISON_FANG, MOVE_WATER_PULSE},
    .abilityNums = 1
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_STUNKY,
    .heldItem = ITEM_LIECHI_BERRY,
    .moves = {MOVE_HEADBUTT, MOVE_PURSUIT, MOVE_POISON_STING, MOVE_POISON_GAS},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Marcel[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BODY_SLAM, MOVE_SHADOW_BALL, MOVE_EARTHQUAKE, MOVE_SEED_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_SCIZOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BULLET_PUNCH, MOVE_POUNCE, MOVE_ROCK_TOMB, MOVE_AERIAL_ACE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alberto[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_RAZOR_WIND, MOVE_HYPER_VOICE, MOVE_HEAT_WAVE, MOVE_STEEL_WING},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_XATU,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_AIR_SLASH, MOVE_SHADOW_BALL, MOVE_GIGA_DRAIN},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Ed[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_ZANGOOSE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_CRUSH_CLAW, MOVE_POISON_JAB, MOVE_NIGHT_SLASH, MOVE_BRICK_BREAK},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_SEVIPER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_DARK_PULSE, MOVE_ICE_FANG, MOVE_GIGA_DRAIN},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntSeafloorCavern4[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GOREBYSS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_ICE_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_THUNDERBOLT, MOVE_SURF, MOVE_NUZZLE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_DEWGONG,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_ICE_BEAM, MOVE_DRILL_RUN, MOVE_PLAY_ROUGH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Declan[] = {
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_CRABHAMMER, MOVE_KNOCK_OFF, MOVE_ICE_BEAM, MOVE_DIG},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntRusturfTunnel[] = {
    {
    .iv = 85,
    .lvl = 15,
    .species = SPECIES_SNORUNT,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_BITE, MOVE_ICE_SHARD, MOVE_HEADBUTT, MOVE_AURORA_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 85,
    .lvl = 15,
    .species = SPECIES_TANGELA,
    .heldItem = ITEM_APICOT_BERRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_LEECH_SEED, MOVE_ACID, MOVE_STUN_SPORE},
    .abilityNums = 1
    },
    {
    .iv = 85,
    .lvl = 15,
    .species = SPECIES_CARVANHA,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_BITE, MOVE_AQUA_JET, MOVE_POISON_FANG, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 85,
    .lvl = 15,
    .species = SPECIES_STUNKY,
    .heldItem = ITEM_LIECHI_BERRY,
    .moves = {MOVE_HEADBUTT, MOVE_PURSUIT, MOVE_SLUDGE, MOVE_BITE},
    .abilityNums = 0
    },
    {
    .iv = 85,
    .lvl = 15,
    .species = SPECIES_PSYDUCK,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_PSYBEAM, MOVE_WATER_PULSE, MOVE_ICY_WIND, MOVE_DISABLE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntWeatherInst1[] = {
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SHADOW_BALL, MOVE_AIR_SLASH, MOVE_WILL_O_WISP, MOVE_KNOCK_OFF},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_CLOYSTER,
    .heldItem = ITEM_NEVER_MELT_ICE,
    .moves = {MOVE_ICICLE_SPEAR, MOVE_ICE_SHARD, MOVE_WATERFALL, MOVE_DRILL_RUN},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_BLACK_GLASSES,
    .moves = {MOVE_WATERFALL, MOVE_KNOCK_OFF, MOVE_BODY_SLAM, MOVE_GUILLOTINE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntWeatherInst2[] = {
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_MUK,
    .heldItem = ITEM_POISON_BARB,
    .moves = {MOVE_POISON_JAB, MOVE_DIG, MOVE_GIGA_DRAIN, MOVE_SHADOW_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_HUNTAIL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_WATERFALL, MOVE_CRUNCH, MOVE_AQUA_JET, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_BRONZONG,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BODY_SLAM, MOVE_EXTRASENSORY, MOVE_ROCK_SLIDE, MOVE_CONFUSE_RAY},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntWeatherInst3[] = {
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_AIR_SLASH, MOVE_HYPNOSIS, MOVE_SHADOW_BALL},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_KINGLER,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_WATERFALL, MOVE_STRENGTH, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_ANCIENT_POWER, MOVE_SURF, MOVE_FACADE, MOVE_PROTECT},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntMuseum1[] = {
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_DRIFLOON,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_GUST, MOVE_HEX, MOVE_WILL_O_WISP, MOVE_ICY_WIND},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ROCK_TOMB, MOVE_SLUDGE, MOVE_SELF_DESTRUCT, MOVE_POISON_GAS},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_CORPHISH,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_AQUA_JET, MOVE_VICE_GRIP, MOVE_AERIAL_ACE, MOVE_DIG},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntMuseum2[] = {
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_GRIMER,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_BULLDOZE, MOVE_SLUDGE, MOVE_POISON_GAS, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_EXEGGCUTE,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_PSYBEAM, MOVE_ANCIENT_POWER, MOVE_HYPNOSIS},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_WAILMER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_HEADBUTT, MOVE_ROLLOUT, MOVE_ICY_WIND},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter1[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre1[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_SKUNTANK,
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre2[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_BRONZONG,
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_KINGLER,
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_BANETTE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre3[] = {
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_GOLDUCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst4[] = {
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_HYPNO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout5[] = {
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_OCTILLERY,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_OCTAZOOKA, MOVE_ICE_BEAM, MOVE_ENERGY_BALL, MOVE_FLAMETHROWER},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_GRUMPIG,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_DARK_PULSE, MOVE_FLASH_CANNON, MOVE_POWER_GEM},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_SHARPEDO,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_STANTLER,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BODY_SLAM, MOVE_ZEN_HEADBUTT, MOVE_ENERGY_BALL, MOVE_MEGAHORN},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_VESPIQUEN,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BUG_BUZZ, MOVE_POWER_GEM, MOVE_AIR_SLASH, MOVE_SLUDGE_BOMB},
    .abilityNums = 1
    },
};

static const struct TrainerMonItemCustomMoves sParty_GruntAquaHideout6[] = {
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_HYDRO_PUMP, MOVE_AIR_SLASH, MOVE_SEED_BOMB, MOVE_WEATHER_BALL},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SCALD, MOVE_ICE_BEAM, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_Fredrick[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_POISON_JAB, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_MACH_PUNCH, MOVE_ROCK_SLIDE, MOVE_ICE_PUNCH},
    .abilityNums = 0
    },
};

static const struct TrainerMonItemCustomMoves sParty_Matt[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_THRASH, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_ICE_SHARD, MOVE_ICICLE_CRASH, MOVE_CRUNCH, MOVE_PSYCHIC_FANGS},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_AGGRON,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_THUNDER, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_IRON_TAIL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DRAGON_CLAW, MOVE_EXTREME_SPEED, MOVE_WATERFALL, MOVE_IRON_HEAD},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_LIECHI_BERRY,
    .moves = {MOVE_LIQUIDATION, MOVE_ICE_FANG, MOVE_CRUNCH, MOVE_SUPERPOWER},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Zander[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_BRICK_BREAK, MOVE_ROCK_SLIDE, MOVE_POISON_JAB, MOVE_KNOCK_OFF},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_ShellyWeatherInstitute[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_ICE_PUNCH, MOVE_DRAIN_PUNCH, MOVE_SHADOW_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_SURF, MOVE_NUZZLE, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_FROSLASS,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_SHADOW_BALL, MOVE_ICE_BEAM, MOVE_DRAINING_KISS, MOVE_DESTINY_BOND},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_DRILL_RUN},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_LIQUIDATION, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_ACID_ARMOR, MOVE_SCALD},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_ShellySeafloorCavern[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_ICE_PUNCH, MOVE_DRAIN_PUNCH, MOVE_SHADOW_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AIR_SLASH, MOVE_MOONBLAST, MOVE_EXTRASENSORY, MOVE_EXTREME_SPEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_FROSLASS,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_SHADOW_BALL, MOVE_ICE_BEAM, MOVE_DRAINING_KISS, MOVE_DESTINY_BOND},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_THUNDERBOLT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_DRAGON_CLAW, MOVE_EXTREME_SPEED, MOVE_WATERFALL, MOVE_IRON_HEAD},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_RECOVER, MOVE_MOONBLAST},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Archie[] = {
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_CANNON, MOVE_DOUBLE_EDGE, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_MAMOSWINE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_ICICLE_CRASH, MOVE_ICE_SHARD, MOVE_THRASH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_AQUA_TAIL, MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_THUNDER, MOVE_THUNDER_WAVE, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_OUTRAGE, MOVE_EXTREME_SPEED, MOVE_WATERFALL, MOVE_IRON_HEAD},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SURF, MOVE_DRACO_METEOR, MOVE_ICE_BEAM, MOVE_FLASH_CANNON},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leah[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Daisy[] = {
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_NIDORINA,
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose1[] = {
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_GROTLE,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_CHERRIM,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Felix[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_EXTREME_SPEED},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_EARTHQUAKE, MOVE_SHADOW_BALL, MOVE_SEED_BOMB},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_EARTHQUAKE, MOVE_ENERGY_BALL},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GASTRODON_W,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTH_POWER, MOVE_SURF, MOVE_ICE_BEAM, MOVE_SLUDGE_BOMB},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_ESPEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_CALM_MIND, MOVE_DAZZLING_GLEAM},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_DRAGON_CLAW, MOVE_FLY, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Violet[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_CHERRIM,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_JUMPLUFF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty1[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_STEELIX,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_DRAGON_BREATH, MOVE_ICE_FANG}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_RHYDON,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_ICE_BEAM, MOVE_MEGAHORN}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Chip[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_HIPPOWDON,
    .moves = {MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_FIRE_FANG, MOVE_THUNDER_FANG}
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_ARMALDO,
    .moves = {MOVE_X_SCISSOR, MOVE_ROCK_SLIDE, MOVE_BRICK_BREAK, MOVE_AERIAL_ACE}
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_AERIAL_ACE, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Foster[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_RHYPERIOR,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_THUNDER_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_FLYGON,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_CRUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty2[] = {
    {
    .iv = 60,
    .lvl = 27,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty3[] = {
    {
    .iv = 70,
    .lvl = 30,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty4[] = {
    {
    .iv = 80,
    .lvl = 33,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty5[] = {
    {
    .iv = 90,
    .lvl = 36,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy1[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_GASTLY,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_TRAPINCH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy2[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_MAGNEZONE,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_FLYGON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy3[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_MAGNEZONE,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_FLYGON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy4[] = {
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy5[] = {
    {
    .iv = 250,
    .lvl = 36,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 250,
    .lvl = 36,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_GabbyAndTy6[] = {
    {
    .iv = 250,
    .lvl = 39,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SONIC_BOOM, MOVE_THUNDER_WAVE, MOVE_METAL_SOUND, MOVE_THUNDERBOLT}
    },
    {
    .iv = 250,
    .lvl = 39,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_ASTONISH, MOVE_STOMP, MOVE_SUPERSONIC, MOVE_HYPER_VOICE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola1[] = {
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_HORSEA,
    },
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_FINNEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Austina[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gwen[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky1[] = {
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_SWINUB,
    .moves = {MOVE_BULLDOZE, MOVE_ICE_SHARD, MOVE_MUD_SLAP, MOVE_ICY_WIND}
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_STARYU,
    .moves = {MOVE_CONFUSION, MOVE_BUBBLE_BEAM, MOVE_SWIFT, MOVE_RAPID_SPIN}
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_BIBAREL,
    .moves = {MOVE_ROLLOUT, MOVE_BITE, MOVE_HEADBUTT, MOVE_AQUA_JET}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Simon[] = {
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_SHELLDER,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_WARTORTLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Charlie[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky3[] = {
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky4[] = {
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Randall[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_CROBAT,
    .heldItem = ITEM_POISON_BARB,
    .moves = {MOVE_POISON_FANG, MOVE_DUAL_WINGBEAT, MOVE_GIGA_DRAIN, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_EXPLOSION, MOVE_AGILITY, MOVE_ROLLOUT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_QUICK_ATTACK, MOVE_AGILITY, MOVE_WING_ATTACK, MOVE_FACADE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Parker[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_TEETER_DANCE, MOVE_DIZZY_PUNCH, MOVE_FOCUS_PUNCH, MOVE_PSYCHIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_CONFUSE_RAY, MOVE_THUNDER_WAVE, MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_DIZZY_PUNCH, MOVE_MACH_PUNCH, MOVE_ICE_PUNCH, MOVE_DYNAMIC_PUNCH},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_George[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_RECOVER, MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_CALM_MIND},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_ENERGY_ROOT,
    .moves = {MOVE_GIGA_DRAIN, MOVE_LEECH_SEED, MOVE_SYNTHESIS, MOVE_SLUDGE_BOMB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SOFT_BOILED, MOVE_SEISMIC_TOSS, MOVE_PROTECT, MOVE_TOXIC},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Berke[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_GUILLOTINE, MOVE_CRABHAMMER, MOVE_NIGHT_SLASH, MOVE_AERIAL_ACE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HORN_DRILL, MOVE_FISSURE, MOVE_EARTHQUAKE, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_ABOMASNOW,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_BLIZZARD, MOVE_SHEER_COLD, MOVE_SEED_BOMB, MOVE_EARTHQUAKE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossSpenser[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_CROBAT,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_FLY, MOVE_GUNK_SHOT, MOVE_GIGA_DRAIN, MOVE_CRUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_LIGHT_SCREEN, MOVE_COSMIC_POWER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ICE_BEAM, MOVE_SURF, MOVE_DRILL_RUN, MOVE_THUNDERBOLT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EXTREME_SPEED, MOVE_FLARE_BLITZ, MOVE_WILD_CHARGE, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_GIGA_IMPACT, MOVE_EARTHQUAKE, MOVE_SHADOW_CLAW, MOVE_SLACK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FRENZY_PLANT, MOVE_SLUDGE_WAVE, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Vincent[] = {
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_HEADLONG_RUSH, MOVE_THUNDER_PUNCH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_DRACO_METEOR, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_EXEGGUTOR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_ENERGY_BALL, MOVE_PSYCHIC, MOVE_EARTHQUAKE, MOVE_SLUDGE_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_PERSIAN,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_SHADOW_CLAW, MOVE_PLAY_ROUGH, MOVE_FAKE_OUT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_DRAGON_CLAW, MOVE_FLAMETHROWER, MOVE_IRON_HEAD, MOVE_FLY},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM, MOVE_THUNDERBOLT},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Leroy[] = {
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_THUNDER_WAVE, MOVE_SHADOW_BALL, MOVE_HYPER_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_DODRIO,
    .heldItem = ITEM_SHARP_BEAK,
    .moves = {MOVE_DRILL_PECK, MOVE_DOUBLE_EDGE, MOVE_TRIPLE_KICK, MOVE_DRILL_RUN},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HYDRO_PUMP, MOVE_FLASH_CANNON, MOVE_EARTHQUAKE, MOVE_ICE_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ENERGY_BALL, MOVE_ANCIENT_POWER, MOVE_EARTHQUAKE, MOVE_SLUDGE_BOMB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FLARE_BLITZ, MOVE_EXTREME_SPEED, MOVE_WILD_CHARGE, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_PSYCHIC_FANGS, MOVE_EARTHQUAKE},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton1[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_TEDDIURSA,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_SHUPPET,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Edgar[] = {
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_CACTURNE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_LEAF_BLADE, MOVE_SHADOW_STRIKE, MOVE_GIGA_DRAIN, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_SPIRITOMB,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_DARK_PULSE, MOVE_SHADOW_BALL, MOVE_CONFUSE_RAY, MOVE_WILL_O_WISP},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_FLASH_CANNON},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_EARTHQUAKE, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_GIRAFARIG,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_HYPER_VOICE, MOVE_SHADOW_BALL, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Albert[] = {
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_MUK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_GUNK_SHOT, MOVE_EARTHQUAKE, MOVE_GIGA_DRAIN, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_YANMEGA,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_BUG_BUZZ, MOVE_AIR_SLASH, MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_DOUBLE_EDGE, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_SHADOW_SNEAK, MOVE_ICE_PUNCH, MOVE_LEECH_LIFE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_MAGMORTAR,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_HEAT_WAVE, MOVE_PSYCHIC, MOVE_SOLAR_BEAM, MOVE_THUNDERBOLT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_EXPLOUD,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_BOOMBURST, MOVE_SHADOW_BALL, MOVE_EXTRASENSORY, MOVE_ICE_BEAM},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Samuel[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_BRAVE_BIRD, MOVE_DOUBLE_EDGE, MOVE_STEEL_WING, MOVE_QUICK_ATTACK},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_ROCK_SLIDE, MOVE_ICE_FANG},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PSYCHIC, MOVE_ICE_BEAM, MOVE_THUNDERBOLT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_SEVIPER,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_SEED_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_BLAZE_KICK, MOVE_THRASH, MOVE_MEGAHORN, MOVE_DRILL_RUN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_SHADOW_BALL, MOVE_REFLECT},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Vito[] = {
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_DODRIO,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_DRILL_PECK, MOVE_DRILL_RUN, MOVE_DOUBLE_EDGE, MOVE_TRIPLE_KICK},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_SHIFTRY,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_LEAF_BLADE, MOVE_ROCK_SLIDE, MOVE_EXTRASENSORY},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PSYCHIC, MOVE_ICE_BEAM, MOVE_THUNDERBOLT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EXTREME_SPEED, MOVE_FLARE_BLITZ, MOVE_WILD_CHARGE, MOVE_OUTRAGE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_MIRROR_COAT, MOVE_EXPLOSION, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossNoland[] = {
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_HIPPOWDON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_SLACK_OFF, MOVE_SPIKES},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_SANDSLASH,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_LEECH_LIFE, MOVE_POISON_JAB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_RHYPERIOR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EARTHQUAKE, MOVE_HEAD_SMASH, MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_FIRE_PUNCH, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_PINSIR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_LEECH_LIFE, MOVE_CRUNCH, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_FLAMETHROWER, MOVE_EARTH_POWER, MOVE_CURSE, MOVE_YAWN},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Warren[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_ARBOK,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Mary[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_PERSIAN,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_AERIAL_ACE, MOVE_FAINT_ATTACK, MOVE_SLASH, MOVE_METAL_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAINT_ATTACK, MOVE_SHOCK_WAVE, MOVE_SWIFT, MOVE_DISARMING_VOICE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHOCK_WAVE, MOVE_MAGICAL_LEAF, MOVE_PSYCHIC, MOVE_DISARMING_VOICE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alexia[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_IRON_HEAD, MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SHUCKLE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_TOXIC, MOVE_ROLLOUT, MOVE_PROTECT, MOVE_DEFENSE_CURL},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_CLOYSTER,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SCALD, MOVE_ICICLE_SPEAR, MOVE_PROTECT, MOVE_SPIKES},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Jody[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_GIGA_IMPACT, MOVE_EARTHQUAKE, MOVE_SHADOW_CLAW, MOVE_BRICK_BREAK},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SPORE, MOVE_MACH_PUNCH, MOVE_SEED_BOMB, MOVE_ICE_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_ABSOL,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_SLASH, MOVE_IRON_HEAD, MOVE_AERIAL_ACE},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Wendy[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_ROCK_SLIDE, MOVE_ICE_FANG}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_ROSERADE,
    .moves = {MOVE_ENERGY_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM, MOVE_SHADOW_BALL}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_LAPRAS,
    .moves = {MOVE_ICE_BEAM, MOVE_SURF, MOVE_THUNDERBOLT, MOVE_DRILL_RUN}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_HEAT_WAVE, MOVE_DARK_PULSE, MOVE_THUNDER_FANG, MOVE_SOLAR_BEAM}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_CROBAT,
    .moves = {MOVE_FLY, MOVE_GUNK_SHOT, MOVE_NIGHT_SLASH, MOVE_GIGA_DRAIN}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Keira[] = {
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_MOONBLAST, MOVE_ICE_BEAM, MOVE_RECOVER},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_BRAVE_BIRD, MOVE_DOUBLE_EDGE, MOVE_STEEL_WING},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_DARK_PULSE, MOVE_HEAT_WAVE, MOVE_SOLAR_BEAM, MOVE_PSYCHIC_FANGS},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_MEGANIUM,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_WAVE, MOVE_MOONBLAST, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_POISON_JAB, MOVE_CLOSE_COMBAT, MOVE_NIGHT_SLASH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke1[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_CHIMECHO,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_MOTHIM,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossGreta[] = {
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_UMBREON,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_DARK_PULSE, MOVE_REST, MOVE_PSYCHIC, MOVE_TOXIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SLUDGE_WAVE, MOVE_SHADOW_BALL, MOVE_DAZZLING_GLEAM, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_MACH_PUNCH, MOVE_BULLET_SEED, MOVE_SPORE, MOVE_ROCK_TOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_MACH_PUNCH, MOVE_ROCK_SLIDE, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_HERACROSS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_MEGAHORN, MOVE_CLOSE_COMBAT, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HI_JUMP_KICK, MOVE_ZEN_HEADBUTT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Hope[] = {
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_UMBREON,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FOUL_PLAY, MOVE_PSYCHIC, MOVE_CURSE, MOVE_TOXIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_MACH_PUNCH, MOVE_ROCK_SLIDE, MOVE_BRICK_BREAK},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_WAVE, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_EARTHQUAKE, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_ABSOL,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_IRON_HEAD, MOVE_PSYCHO_CUT, MOVE_ICE_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_EARTH_POWER, MOVE_AMNESIA, MOVE_ROCK_SLIDE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Shannon[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_DRIFBLIM,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SHADOW_BALL, MOVE_AIR_SLASH, MOVE_DESTINY_BOND, MOVE_HAZE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HYDRO_PUMP, MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_SEED_BOMB, MOVE_ICE_PUNCH, MOVE_MACH_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_AGGRON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HEAD_SMASH, MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_ICE_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_ARBOK,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_GUNK_SHOT, MOVE_DRAGON_RUSH, MOVE_CRUNCH, MOVE_PSYCHIC_FANGS},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_CURSE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Michelle[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HYDRO_PUMP, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_MYSTICAL_FIRE, MOVE_FOCUS_BLAST},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_FLAREON,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_FLARE_BLITZ, MOVE_QUICK_ATTACK, MOVE_DIG, MOVE_BODY_SLAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_SHADOW_BALL, MOVE_GIGA_DRAIN, MOVE_THUNDERBOLT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_IRON_TAIL, MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_THUNDER_FANG},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Caroline[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_MILTANK,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BODY_SLAM, MOVE_PLAY_ROUGH, MOVE_MILK_DRINK, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_OCTILLERY,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_OCTAZOOKA, MOVE_ICE_BEAM, MOVE_MUD_SHOT, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ERUPTION, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_CACTURNE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_LEAF_BLADE, MOVE_SHADOW_STRIKE, MOVE_SHADOW_SNEAK, MOVE_BRICK_BREAK},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_ROTOM_FROST,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DUAL_WINGBEAT, MOVE_FIRE_FANG, MOVE_CRABHAMMER},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Julie[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SANDSLASH,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_X_SCISSOR, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_DRAGON_PULSE, MOVE_ENERGY_BALL, MOVE_ANCIENT_POWER, MOVE_GROWTH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GRANBULL,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PLAY_ROUGH, MOVE_BRICK_BREAK, MOVE_THUNDER_FANG, MOVE_ICE_FANG},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_LIQUIDATION, MOVE_AQUA_JET, MOVE_CRUNCH, MOVE_ICE_FANG},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_NINETALES,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FLAMETHROWER, MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_EXTRASENSORY},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_MOONBLAST, MOVE_SHADOW_BALL, MOVE_MYSTICAL_FIRE},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke5[] = {
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Patricia[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_DRAIN_PUNCH, MOVE_ICE_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_ESPEON,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_DAZZLING_GLEAM, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_PSYCHIC, MOVE_MOONBLAST, MOVE_SHADOW_BALL, MOVE_MYSTICAL_FIRE},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kindra[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_GENGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tammy[] = {
    {
    .iv = 200,
    .lvl = 53,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .iv = 200,
    .lvl = 53,
    .species = SPECIES_GENGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie1[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_GENGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tasha[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_DUSKNOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SPOINK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie3[] = {
    {
    .iv = 20,
    .lvl = 35,
    .species = SPECIES_SPOINK,
    },
    {
    .iv = 20,
    .lvl = 35,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie4[] = {
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_SPOINK,
    },
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie5[] = {
    {
    .iv = 40,
    .lvl = 42,
    .species = SPECIES_DUSKULL,
    },
    {
    .iv = 40,
    .lvl = 42,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 40,
    .lvl = 42,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy1[] = {
    {
    .iv = 50,
    .lvl = 8,
    .species = SPECIES_SHROOMISH,
    .heldItem = ITEM_MIRACLE_SEED
    },
    {
    .iv = 50,
    .lvl = 8,
    .species = SPECIES_MEOWTH,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 50,
    .lvl = 8,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_TWISTED_SPOON
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_BUNEARY,
    .heldItem = ITEM_SILK_SCARF
    }
};

static const struct TrainerMonItemCustomMoves sParty_Daphne[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_WEATHER_BALL, MOVE_PSYCHIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_OCTILLERY,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_ICE_BEAM, MOVE_OCTAZOOKA, MOVE_FLASH_CANNON, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_FLOATZEL,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_ICE_FANG, MOVE_DIG, MOVE_CRUNCH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Courtney[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_SKY_ATTACK, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MAGMORTAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_FIRE_BLAST, MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_EARTHQUAKE, MOVE_SEED_BOMB, MOVE_ROCK_SLIDE, MOVE_ZEN_HEADBUTT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MISMAGIUS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_MOONBLAST, MOVE_MYSTICAL_FIRE, MOVE_ENERGY_BALL},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_POISON_JAB, MOVE_BRICK_BREAK, MOVE_IRON_TAIL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EXTREME_SPEED, MOVE_FLARE_BLITZ, MOVE_WILD_CHARGE, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Cindy2[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_TACKLE, MOVE_TAIL_WHIP, MOVE_NONE, MOVE_NONE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Brianna[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_SEAKING,
    .heldItem = ITEM_MYSTIC_WATER
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_CLOYSTER,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_WISE_GLASSES
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_GOLDUCK,
    .heldItem = ITEM_LUM_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Naomi[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy3[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy4[] = {
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy5[] = {
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemCustomMoves sParty_Cindy6[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_FURY_SWIPES, MOVE_MUD_SPORT, MOVE_ODOR_SLEUTH, MOVE_SAND_ATTACK},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Melissa[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_PURUGLY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sheila[] = {
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shirley[] = {
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_VIBRAVA,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_DRIFBLIM,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_JUMPLUFF,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica1[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_ROSERADE,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_ENERGY_BALL, MOVE_SHADOW_BALL, MOVE_PSYCHIC}
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_LOPUNNY,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_BODY_SLAM, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Connie[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_QWILFISH,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bridget[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_SLOWBRO,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_JYNX,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Olivia[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_SEAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DRILL_RUN, MOVE_PLAY_ROUGH, MOVE_AQUA_TAIL, MOVE_MEGAHORN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_KINGLER,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CRABHAMMER, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_X_SCISSOR},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_GIGA_DRAIN, MOVE_FAKE_OUT, MOVE_ICE_BEAM},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tiffany[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_AMPHAROS,
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_GASTRODON_E,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica2[] = {
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FAINT_ATTACK}
    },
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica3[] = {
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FAINT_ATTACK}
    },
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FAINT_ATTACK}
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica5[] = {
    {
    .iv = 40,
    .lvl = 44,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FAINT_ATTACK}
    },
    {
    .iv = 40,
    .lvl = 44,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston1[] = {
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_VULPIX,
    .heldItem = ITEM_CHARCOAL
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_FINNEON,
    .heldItem = ITEM_MYSTIC_WATER
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_CACNEA,
    .heldItem = ITEM_MIRACLE_SEED
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mollie[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Garret[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_MUSCLE_BAND
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston2[] = {
    {
    .iv = 0,
    .lvl = 27,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston3[] = {
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston4[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winston5[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_FURY_SWIPES, MOVE_MUD_SPORT, MOVE_ODOR_SLEUTH, MOVE_SAND_ATTACK},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve1[] = {
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_KANGASKHAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia1[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mark[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_RHYPERIOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtChimney1[] = {
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_MAGMAR,
    },
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_LAIRON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_AGGRON,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_RHYDON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Luis[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dominik[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_HUNTAIL,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_MANTINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Douglas[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darrin[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_CORSOLA,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony1[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_CRAWDAUNT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerome[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_RELICANTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Matthew[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_OCTILLERY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_David[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_BLASTOISE,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_KABUTOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Spencer[] = {
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_KINGLER,
    },
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Roland[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nolen[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_KABUTOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Stan[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_QWILFISH,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_HUNTAIL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Barry[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dean[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rodney[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Richard[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Herman[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_HUNTAIL,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Santiago[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gilbert[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Franklin[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kevin[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GOLDUCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jack[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dudley[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chad[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Takao[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hitoshi[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kiyo[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GALLADE,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koichi[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob1[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob2[] = {
    {
    .iv = 110,
    .lvl = 27,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob4[] = {
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Nob5[] = {
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_MACHOP,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_MACHOKE,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_MACHOKE,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_BLACK_BELT
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Yuji[] = {
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Daisuke[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Atsushi[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Kirk[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_ELECTRIKE,
    .moves = {MOVE_QUICK_ATTACK, MOVE_THUNDER_FANG, MOVE_SPARK, MOVE_BITE}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_ELEKID,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_LOW_KICK, MOVE_THUNDER_WAVE, MOVE_LOW_KICK}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_FLAAFFY,
    .moves = {MOVE_TAKE_DOWN, MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout7[] = {
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_CLEFABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout8[] = {
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_KINGLER,
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 200,
    .lvl = 63,
    .species = SPECIES_LAPRAS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shawn[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_LUXIO,
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_JOLTEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando1[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_LUXRAY,
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton1[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_TRAPINCH,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_CHATOT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton2[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_WHISMUR,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cole[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MAGMAR,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_NIDOKING,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jeff[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MAGCARGO,
    .moves = {MOVE_FLAMETHROWER, MOVE_ANCIENT_POWER, MOVE_BODY_SLAM, MOVE_RECOVER}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_FLAMETHROWER, MOVE_HYPER_VOICE, MOVE_BRICK_BREAK, MOVE_SHADOW_BALL}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SUNFLORA,
    .moves = {MOVE_HEAT_WAVE, MOVE_ENERGY_BALL, MOVE_SYNTHESIS, MOVE_GROWTH}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SKUNTANK,
    .moves = {MOVE_FLAMETHROWER, MOVE_CRUNCH, MOVE_POISON_JAB, MOVE_EXPLOSION}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Axle[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_STANTLER,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_RAPIDASH,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_LICKILICKY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jace[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_HITMONTOP,
    .moves = {MOVE_BLAZE_KICK, MOVE_CLOSE_COMBAT, MOVE_FAKE_OUT, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CLAYDOL,
    .moves = {MOVE_MYSTICAL_FIRE, MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_SHADOW_BALL}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_THUNDER_FANG, MOVE_PSYCHIC_FANGS}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Keegan[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_NINETALES,
    .moves = {MOVE_FLAMETHROWER, MOVE_SHADOW_BALL, MOVE_EXTRASENSORY, MOVE_WILL_O_WISP}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_GRANBULL,
    .moves = {MOVE_PLAY_ROUGH, MOVE_BRICK_BREAK, MOVE_FIRE_FANG, MOVE_PSYCHIC_FANGS}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_AGGRON,
    .moves = {MOVE_FLAMETHROWER, MOVE_IRON_HEAD, MOVE_ROCK_SLIDE, MOVE_ICE_BEAM}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_BANETTE,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_NIGHT_SLASH, MOVE_POISON_JAB, MOVE_SHADOW_SNEAK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie1[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_TORKOAL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Drew[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_NIDOKING,
    .moves = {MOVE_EARTHQUAKE, MOVE_POISON_JAB, MOVE_SHADOW_BALL, MOVE_ICE_BEAM}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_WHISCASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_SURF, MOVE_ICE_BEAM, MOVE_ZEN_HEADBUTT}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_GOLEM,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_THUNDER_PUNCH, MOVE_FIRE_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Beau[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CLAYDOL,
    .moves = {MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_COSMIC_POWER, MOVE_RAPID_SPIN}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_DONPHAN,
    .moves = {MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_ICE_FANG, MOVE_FIRE_FANG}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_HIPPOWDON,
    .moves = {MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_ICE_FANG, MOVE_FIRE_FANG}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Larry[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_HAUNTER,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_NIDORINO,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_ZANGOOSE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shane[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_RAMPARDOS,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_BASTIODON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Justin[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_KECLEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan1[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_FARFETCHD,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_DONPHAN,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_CACTURNE,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_LUXRAY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Autumn[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_URSARING,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_TANGROWTH
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Travis[] = {
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_VIGOROTH,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_STUNKY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_LINOONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_LINOONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brent[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Donald[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_HERACROSS,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_VESPIQUEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Taylor[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_YANMEGA,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_LEDIAN,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_PARASECT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey1[] = {
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_PARASECT,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Derek[] = {
    {
    .iv = 150,
    .lvl = 23,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 150,
    .lvl = 23,
    .species = SPECIES_BEAUTIFLY,
    },
    {
    .iv = 150,
    .lvl = 23,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SURSKIT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey3[] = {
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Jeffrey5[] = {
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_SURSKIT,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_DUSTOX,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_SURSKIT,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_MASQUERAIN,
    .heldItem = ITEM_SILVER_POWDER
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_BEAUTIFLY,
    .heldItem = ITEM_NONE
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Edward[] = {
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_KADABRA,
    .moves = {MOVE_CONFUSION, MOVE_PSYBEAM, MOVE_SIGNAL_BEAM, MOVE_KNOCK_OFF}
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_SLOWPOKE,
    .moves = {MOVE_PSYBEAM, MOVE_WATER_PULSE, MOVE_HEADBUTT, MOVE_YAWN}
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_MR_MIME,
    .moves = {MOVE_PSYBEAM, MOVE_DISARMING_VOICE, MOVE_LIGHT_SCREEN, MOVE_REFLECT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Preston[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GALLADE,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GIRAFARIG,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Virgil[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_BRONZONG,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_HYPNO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Blake[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_WOBBUFFET,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_William[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_GARDEVOIR,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_GALLADE,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Joshua[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GRUMPIG,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_DARK_PULSE, MOVE_POWER_GEM, MOVE_FLASH_CANNON},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron1[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_LUNATONE,
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron2[] = {
    {
    .iv = 10,
    .lvl = 33,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 33,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron3[] = {
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jaclyn[] = {
    {
    .iv = 0,
    .lvl = 16,
    .species = SPECIES_ABRA,
    .moves = {MOVE_HIDDEN_POWER, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hannah[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_LUNATONE,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_GARDEVOIR,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Samantha[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_SLOWBRO,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_EXEGGUTOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maura[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GARDEVOIR,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_XATU,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kayla[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_ESPEON,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_XATU,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_SLOWBRO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alexis[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GIRAFARIG,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_HYPER_VOICE, MOVE_DAZZLING_GLEAM, MOVE_THUNDERBOLT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_ICE_BEAM, MOVE_SHADOW_BALL, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_PSYCHIC, MOVE_POWER_GEM, MOVE_MOONBLAST, MOVE_ICE_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki1[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki4[] = {
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_LUNATONE,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Walter1[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_LUXRAY,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_ARCANINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Micah[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_HOUNDOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thomas[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARCANINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Walter2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_GOLDUCK,
    .moves = {MOVE_FURY_SWIPES, MOVE_DISABLE, MOVE_CONFUSION, MOVE_PSYCH_UP}
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Sidney[] = {
    {
    .iv = 255,
    .evs = {252, 0, 4, 0, 252, 0},
    .lvl = 95,
    .species = SPECIES_UMBREON,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FOUL_PLAY, MOVE_PROTECT, MOVE_CURSE, MOVE_TOXIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 95,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 95,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_NIGHT_SLASH, MOVE_THUNDER_WAVE, MOVE_SUPERPOWER},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 252, 0, 4, 0, 252},
    .lvl = 96,
    .species = SPECIES_SHIFTRY,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_LEAF_BLADE, MOVE_KNOCK_OFF, MOVE_ROCK_SLIDE, MOVE_EXTRASENSORY},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {132, 252, 0, 0, 0, 124},
    .lvl = 96,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CRUNCH, MOVE_HEAD_SMASH, MOVE_EARTHQUAKE, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 96,
    .species = SPECIES_DRAPION,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_POISON_TAIL, MOVE_EARTHQUAKE, MOVE_ICE_FANG},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Phoebe[] = {
    {
    .iv = 255,
    .evs = {248, 0, 252, 0, 8, 0},
    .lvl = 96,
    .species = SPECIES_SPIRITOMB,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SHADOW_BALL, MOVE_DARK_PULSE, MOVE_NASTY_PLOT, MOVE_WILL_O_WISP},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {252, 112, 144, 0, 0, 0},
    .lvl = 96,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_PUNCH, MOVE_BRICK_BREAK, MOVE_EARTHQUAKE, MOVE_WILL_O_WISP},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 96,
    .species = SPECIES_MISMAGIUS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SHADOW_BALL, MOVE_MOONBLAST, MOVE_THUNDERBOLT, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 97,
    .species = SPECIES_BANETTE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_SHADOW_SNEAK, MOVE_NIGHT_SLASH, MOVE_GUNK_SHOT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {252, 252, 0, 0, 4, 0},
    .lvl = 97,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_HYDRO_PUMP, MOVE_REST, MOVE_WILL_O_WISP},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 97,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_WAVE, MOVE_GIGA_DRAIN, MOVE_FOCUS_BLAST},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Glacia[] = {
    {
    .iv = 255,
    .evs = {248, 0, 8, 0, 252, 0},
    .lvl = 97,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BLIZZARD, MOVE_SURF, MOVE_EARTHQUAKE, MOVE_TOXIC},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 97,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_ICICLE_CRASH, MOVE_ICE_SHARD, MOVE_CRUNCH, MOVE_ZEN_HEADBUTT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {80, 0, 0, 252, 0, 176},
    .lvl = 97,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ICE_BEAM, MOVE_SURF, MOVE_THUNDERBOLT, MOVE_DRILL_RUN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {4, 0, 0, 252, 0, 252},
    .lvl = 98,
    .species = SPECIES_FROSLASS,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_ICE_BEAM, MOVE_SHADOW_BALL, MOVE_SPIKES, MOVE_DESTINY_BOND},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 98,
    .species = SPECIES_ABOMASNOW,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_BLIZZARD, MOVE_GIGA_DRAIN, MOVE_FOCUS_BLAST, MOVE_ICE_SHARD},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 98,
    .species = SPECIES_GLACEON,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_ICE_BEAM, MOVE_SIGNAL_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Drake[] = {
    {
    .iv = 255,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 98,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EXTREME_SPEED, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_FIRE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 98,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_EARTHQUAKE, MOVE_OUTRAGE, MOVE_FIRE_PUNCH, MOVE_AERIAL_ACE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 0, 4, 252, 0, 252},
    .lvl = 98,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLAMETHROWER, MOVE_DRAGON_PULSE, MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 99,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HEAD_SMASH, MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_ICE_FANG},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {4, 0, 0, 252, 0, 252},
    .lvl = 99,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_DRACO_METEOR, MOVE_OCTAZOOKA, MOVE_ICE_BEAM, MOVE_FLASH_CANNON},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {44, 252, 0, 0, 0, 212},
    .lvl = 99,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_DRAGON_CLAW, MOVE_FLY, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne1[] = {
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_LILEEP,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_ACID, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_ANORITH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_CUT, MOVE_ROCK_TOMB, MOVE_METAL_CLAW, MOVE_AQUA_JET},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_ROCK_TOMB, MOVE_SEISMIC_TOSS, MOVE_YAWN},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_ARON,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_ROCK_TOMB, MOVE_METAL_CLAW, MOVE_HEADBUTT, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_CRANIDOS,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_HEADBUTT, MOVE_ROCK_TOMB, MOVE_ROCK_SMASH, MOVE_PURSUIT},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 17,
    .species = SPECIES_LARVITAR,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_BITE, MOVE_ROCK_SLIDE, MOVE_QUICK_ATTACK, MOVE_BULLDOZE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly1[] = {
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_ROCK_TOMB, MOVE_ROLLING_KICK, MOVE_MACH_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_MONFERNO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_MACH_PUNCH, MOVE_FIRE_PUNCH, MOVE_FAKE_OUT, MOVE_ROCK_TOMB},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_MAKUHITA,
    .heldItem = ITEM_BLACK_BELT,
    .moves = {MOVE_VITAL_THROW, MOVE_ROCK_TOMB, MOVE_REVERSAL, MOVE_BULK_UP},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_CROAGUNK,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_KNOCK_OFF, MOVE_ROCK_TOMB, MOVE_MACH_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_MAROWAK,
    .heldItem = ITEM_THICK_CLUB,
    .moves = {MOVE_BONEMERANG, MOVE_ROCK_TOMB, MOVE_DOUBLE_KICK, MOVE_TAKE_DOWN},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 20,
    .species = SPECIES_HERACROSS,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_BULLET_SEED, MOVE_ROCK_BLAST, MOVE_ARM_THRUST, MOVE_PIN_MISSILE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson1[] = {
    {
    .iv = 200,
    .lvl = 28,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDER_WAVE, MOVE_SHOCK_WAVE, MOVE_BUBBLE_BEAM, MOVE_AURORA_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 28,
    .species = SPECIES_JOLTEON,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER_WAVE, MOVE_SHOCK_WAVE, MOVE_QUICK_ATTACK, MOVE_BITE},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 28,
    .species = SPECIES_ELECTABUZZ,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_MACH_PUNCH, MOVE_THUNDER_PUNCH, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 250,
    .lvl = 28,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_SHOCK_WAVE, MOVE_TRI_ATTACK, MOVE_FLASH_CANNON},
    .abilityNums = 1
    },
    {
    .iv = 250,
    .lvl = 28,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_NUZZLE, MOVE_SHOCK_WAVE, MOVE_DRAINING_KISS, MOVE_SURF},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .lvl = 30,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_ICE_FANG, MOVE_THUNDER_FANG, MOVE_CRUNCH, MOVE_QUICK_ATTACK},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery1[] = {
    {
    .iv = 250,
    .evs = {248, 0, 28, 0, 232, 0},
    .lvl = 41,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_YAWN, MOVE_BODY_SLAM, MOVE_FLAMETHROWER, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 41,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRILL_RUN, MOVE_BLAZE_KICK, MOVE_BODY_SLAM, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 41,
    .species = SPECIES_ROTOM_HEAT,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_WILL_O_WISP, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_PAIN_SPLIT},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 41,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_FIRE_FANG, MOVE_EXTREME_SPEED, MOVE_THUNDER_FANG, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 41,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_FIRE_PUNCH, MOVE_MACH_PUNCH, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 42,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_DRAGON_PULSE, MOVE_EARTHQUAKE, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman1[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_CURSE, MOVE_FACADE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_CHOICE_BAND,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_ZEN_HEADBUTT, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_EXPLOUD,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_VOICE, MOVE_FLAMETHROWER, MOVE_EARTHQUAKE, MOVE_SHADOW_BALL},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_PORYGON_Z,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_PSYCHIC, MOVE_ICE_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HYPER_VOICE, MOVE_RAZOR_WIND, MOVE_HEAT_WAVE, MOVE_QUICK_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 250,
    .lvl = 46,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_GIGA_IMPACT, MOVE_SHADOW_CLAW, MOVE_BRICK_BREAK, MOVE_EARTHQUAKE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona1[] = {
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_SPIKES, MOVE_DRILL_PECK, MOVE_STEEL_WING, MOVE_TOXIC},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_MOONBLAST, MOVE_DRAGON_PULSE, MOVE_EARTHQUAKE, MOVE_SYNTHESIS},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_CROBAT,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_GUNK_SHOT, MOVE_DUAL_WINGBEAT, MOVE_GIGA_DRAIN, MOVE_STEEL_WING},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HEAD_SMASH, MOVE_OUTRAGE, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_ROTOM_FAN,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_SHADOW_BALL, MOVE_AIR_SLASH, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza1[] = {
    {
    .iv = 255,
    .evs = {232, 60, 0, 0, 40, 176},
    .lvl = 70,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_METEOR_MASH, MOVE_PSYCHIC_FANGS, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {252, 4, 0, 0, 252, 0},
    .lvl = 70,
    .species = SPECIES_BRONZONG,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_ROCK_SLIDE, MOVE_REFLECT, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 70,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_CHOICE_SCARF,
    .moves = {MOVE_PSYCHIC, MOVE_MOONBLAST, MOVE_SHADOW_BALL, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 70,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_NIGHT_SLASH, MOVE_LEAF_BLADE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {252, 0, 252, 0, 0, 4},
    .lvl = 70,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FLARE_BLITZ, MOVE_ZEN_HEADBUTT, MOVE_PROTECT, MOVE_MORNING_SUN},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 70,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_POWER_GEM, MOVE_EARTH_POWER, MOVE_PROTECT},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan1[] = {
    {
    .iv = 255,
    .evs = {248, 0, 8, 0, 252, 0},
    .lvl = 76,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WEATHER_BALL, MOVE_ICE_BEAM, MOVE_TOXIC, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 76,
    .species = SPECIES_FLOATZEL,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_BULK_UP, MOVE_BRICK_BREAK},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {40, 0, 220, 0, 248, 0},
    .lvl = 76,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_RAIN_DANCE, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 252, 0, 0, 4, 252},
    .lvl = 76,
    .species = SPECIES_HUNTAIL,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_WATERFALL, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 76,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_WEATHER_BALL, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM, MOVE_RAIN_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {4, 0, 0, 252, 0, 252},
    .lvl = 77,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_MUDDY_WATER, MOVE_DRACO_METEOR, MOVE_ICE_BEAM, MOVE_FLASH_CANNON},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry1[] = {
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_BELLSPROUT,
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_MUNCHLAX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ted[] = {
    {
    .iv = 10,
    .lvl = 17,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paul[] = {
    {
    .iv = 10,
    .lvl = 15,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 10,
    .lvl = 15,
    .species = SPECIES_ODDISH,
    },
    {
    .iv = 10,
    .lvl = 15,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry5[] = {
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen1[] = {
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_CHERUBI,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_SPOINK,
    },
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_SNUBBULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Georgia[] = {
    {
    .iv = 10,
    .lvl = 16,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 16,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen5[] = {
    {
    .iv = 50,
    .lvl = 35,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 50,
    .lvl = 35,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_KateAndJoy[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_ELECTIVIRE,
    .moves = {MOVE_WILD_CHARGE, MOVE_ICE_PUNCH, MOVE_FIRE_PUNCH, MOVE_CLOSE_COMBAT}
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_MAGMORTAR,
    .moves = {MOVE_HEAT_WAVE, MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_SLUDGE_BOMB}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg1[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_GLOOM,
    .moves = {MOVE_MEGA_DRAIN, MOVE_PETAL_BLIZZARD, MOVE_SLUDGE, MOVE_SLEEP_POWDER}
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_WEEPINBELL,
    .moves = {MOVE_RAZOR_LEAF, MOVE_KNOCK_OFF, MOVE_BULLET_SEED, MOVE_TOXIC}
    },
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Victor[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_DROWZEE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_DODUO,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel1[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_BUNEARY,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_ELEKID,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MAGBY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemCustomMoves sParty_King[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_FLAMETHROWER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_SEAKING,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_DRILL_RUN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_KINGLER,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_CRABHAMMER, MOVE_ICE_BEAM, MOVE_DIG, MOVE_BRICK_BREAK},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_SLOWKING,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_SURF, MOVE_ICE_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_GIGA_IMPACT, MOVE_EARTHQUAKE, MOVE_SHADOW_BALL, MOVE_AERIAL_ACE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_DRAGON_PULSE, MOVE_SURF, MOVE_ICE_BEAM, MOVE_FLASH_CANNON},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel2[] = {
    {
    .iv = 0,
    .lvl = 29,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel3[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel4[] = {
    {
    .iv = 0,
    .lvl = 35,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel5[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Victoria[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_CLEFAIRY,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_JIGGLYPUFF,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_TANGELA,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Vanessa[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_QUICK_CLAW
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Bethany[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_VAPOREON,
    .heldItem = ITEM_LEFTOVERS
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_LUM_BERRY
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_QUICK_CLAW
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel1[] = {
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_CHATOT,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_RALTS,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Timothy1[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_HITMONCHAN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy2[] = {
    {
    .iv = 210,
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy3[] = {
    {
    .iv = 220,
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy4[] = {
    {
    .iv = 230,
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy5[] = {
    {
    .iv = 240,
    .lvl = 42,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Vicky[] = {
    {
    .iv = 200,
    .lvl = 26,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_HI_JUMP_KICK, MOVE_CALM_MIND, MOVE_ZEN_HEADBUTT, MOVE_DETECT}
    },
    {
    .iv = 200,
    .lvl = 26,
    .species = SPECIES_SPINDA,
    .moves = {MOVE_BODY_SLAM, MOVE_DIZZY_PUNCH, MOVE_PSYBEAM, MOVE_FAKE_OUT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby1[] = {
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 200,
    .lvl = 38,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby2[] = {
    {
    .iv = 210,
    .lvl = 30,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 210,
    .lvl = 30,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby3[] = {
    {
    .iv = 220,
    .lvl = 33,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 220,
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby4[] = {
    {
    .iv = 230,
    .lvl = 36,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 230,
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby5[] = {
    {
    .iv = 240,
    .lvl = 39,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 240,
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin1[] = {
    {
    .iv = 20,
    .lvl = 5,
    .species = SPECIES_BIDOOF,
    },
    {
    .iv = 20,
    .lvl = 5,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 20,
    .lvl = 5,
    .species = SPECIES_RATTATA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Billy[] = {
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_SENTRET,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_HOOTHOOT,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_SPEAROW,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_MANKEY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Josh[] = {
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_GEODUDE,
    .moves = {MOVE_TACKLE, MOVE_ROCK_THROW, MOVE_BULLDOZE, MOVE_ROLLOUT}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_CACNEA,
    .moves = {MOVE_BULLET_SEED, MOVE_FAINT_ATTACK, MOVE_POISON_STING, MOVE_LEECH_SEED}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_PINECO,
    .moves = {MOVE_TACKLE, MOVE_PROTECT, MOVE_POUNCE, MOVE_ROCK_TOMB}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_KABUTO,
    .moves = {MOVE_AQUA_JET, MOVE_ROLLOUT, MOVE_MUD_SHOT, MOVE_ABSORB}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Tommy[] = {
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_OMANYTE,
    .moves = {MOVE_WATER_PULSE, MOVE_ROLLOUT, MOVE_MUD_SHOT, MOVE_ROCK_TOMB}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_ZUBAT,
    .moves = {MOVE_MEGA_DRAIN, MOVE_BITE, MOVE_POISON_FANG, MOVE_WING_ATTACK}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_RHYHORN,
    .moves = {MOVE_HORN_ATTACK, MOVE_ROCK_TOMB, MOVE_BULLDOZE, MOVE_SECRET_POWER}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_SHIELDON,
    .moves = {MOVE_PROTECT, MOVE_ROCK_TOMB, MOVE_SPIKE_CANNON, MOVE_SECRET_POWER}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Joey[] = {
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_RATTATA,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_STARLY,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_MAREEP,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ben[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    .moves = {MOVE_SHOCK_WAVE, MOVE_SWIFT, MOVE_SPARK, MOVE_THUNDER_WAVE}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_ROTOM,
    .moves = {MOVE_SHOCK_WAVE, MOVE_SHADOW_SNEAK, MOVE_SPARK, MOVE_ASTONISH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Quincy[] = {
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_ICE_PUNCH, MOVE_SHADOW_SNEAK, MOVE_LEECH_LIFE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_RHYPERIOR,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SURF, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_CURSE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE, MOVE_SLUDGE_BOMB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_FLARE_BLITZ, MOVE_POWER_GEM, MOVE_SHADOW_BALL, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_PORYGON_Z,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_PSYCHIC},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Katelynn[] = {
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_MOONBLAST, MOVE_SHADOW_BALL, MOVE_MYSTICAL_FIRE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_GIGA_IMPACT, MOVE_EARTHQUAKE, MOVE_SHADOW_BALL, MOVE_AERIAL_ACE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_BLASTOISE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_HYDRO_CANNON, MOVE_ROCK_SLIDE, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_VICTREEBEL,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_LEAF_BLADE, MOVE_POISON_JAB, MOVE_KNOCK_OFF, MOVE_BODY_SLAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_NINETALES,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLAMETHROWER, MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_CALM_MIND},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_HERACROSS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_MEGAHORN, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jaylen[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_TRAPINCH,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dillon[] = {
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_NOCTOWL,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_DONPHAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_BIBAREL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Eddie[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MEOWTH,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Allen[] = {
    {
    .iv = 20,
    .lvl = 6,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 20,
    .lvl = 6,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 20,
    .lvl = 6,
    .species = SPECIES_NIDORAN_M,
    },
    {
    .iv = 20,
    .lvl = 6,
    .species = SPECIES_SHINX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Timmy[] = {
    {
    .iv = 80,
    .lvl = 20,
    .species = SPECIES_SNORUNT,
    },
    {
    .iv = 80,
    .lvl = 20,
    .species = SPECIES_BRONZOR,
    },
    {
    .iv = 80,
    .lvl = 20,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wallace[] = {
    {
    .iv = 255,
    .evs = {0, 0, 252, 4, 252, 0},
    .lvl = 100,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYDRO_PUMP, MOVE_DOUBLE_EDGE, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .evs = {0, 252, 4, 0, 0, 252},
    .lvl = 100,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_SUPERPOWER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {0, 0, 0, 252, 4, 252},
    .lvl = 100,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SURF, MOVE_ICE_BEAM, MOVE_RAIN_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {4, 0, 0, 252, 0, 252},
    .lvl = 100,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_DRACO_METEOR, MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_SURF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {252, 0, 108, 0, 148, 0},
    .lvl = 100,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_MOONBLAST, MOVE_ICE_BEAM, MOVE_RECOVER},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .evs = {252, 0, 108, 0, 148, 0},
    .lvl = 100,
    .species = SPECIES_SUICUNE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SCALD, MOVE_ICE_BEAM, MOVE_TOXIC, MOVE_MIRROR_COAT},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andrew[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_FEEBAS,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MANTYKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ivan[] = {
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_HORSEA,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_WOOPER,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_SHELLOS_E,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Claude[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_POLIWHIRL,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot1[] = {
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_CLAMPERL,
    },
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_PSYDUCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ned[] = {
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_HORSEA,
    },
    {
    .iv = 75,
    .lvl = 15,
    .species = SPECIES_MANTYKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dale[] = {
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_POLIWHIRL,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_SEEL,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_SHELLDER,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_CORPHISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nolan[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_OCTILLERY,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Barny[] = {
    {
    .iv = 150,
    .lvl = 40,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 150,
    .lvl = 40,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 150,
    .lvl = 40,
    .species = SPECIES_OCTILLERY,
    },
    {
    .iv = 150,
    .lvl = 40,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wade[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_QUAGSIRE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carter[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_QWILFISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot2[] = {
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 31,
    .lvl = 31,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ronald[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacob[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_LUXIO,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_ELECTABUZZ,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Anthony[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_PIKACHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin1[] = {
    {
    .iv = 200,
    .lvl = 24,
    .species = SPECIES_MURKROW,
    },
    {
    .iv = 200,
    .lvl = 24,
    .species = SPECIES_NOCTOWL,
    },
    {
    .iv = 200,
    .lvl = 24,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin5[] = {
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail1[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_BRONZOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jasmine[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_FLAAFFY,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_ROTOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan1[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_FEAROW,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria1[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_PIDGEOTTO,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_STARAVIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Camden[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Demetrius[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_URSARING,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_YANMEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah1[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_KABUTOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo1[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_HUNTAIL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chase[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah3[] = {
    {
    .iv = 20,
    .lvl = 42,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah4[] = {
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah5[] = {
    {
    .iv = 40,
    .lvl = 48,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isobel[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Donny[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_GOREBYSS,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Talia[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn1[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_VAPOREON,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Allison[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SLOWBRO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn3[] = {
    {
    .iv = 20,
    .lvl = 42,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn4[] = {
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn5[] = {
    {
    .iv = 40,
    .lvl = 48,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas1[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GARCHOMP,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_DRAGONITE,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas2[] = {
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_GARCHOMP,
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_DRAGONITE,
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas3[] = {
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_GARCHOMP,
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_DRAGONITE,
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas4[] = {
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_GARCHOMP,
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_DRAGONITE,
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Nicolas5[] = {
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_GARCHOMP,
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_DRAGONITE,
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_SALAMENCE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Aaron[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SALAMENCE,
    .moves = {MOVE_OUTRAGE, MOVE_IRON_HEAD, MOVE_FLAMETHROWER, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Perry[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hugh[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Phil[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_STARAPTOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jared[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_FEAROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Humberto[] = {
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Presley[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwardo[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_DELIBIRD,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_GLISCOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Colin[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert1[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_HONCHKROW,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benny[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chester[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_STARAPTOR,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_NOCTOWL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert2[] = {
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_NATU,
    },
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert3[] = {
    {
    .iv = 20,
    .lvl = 35,
    .species = SPECIES_NATU,
    },
    {
    .iv = 20,
    .lvl = 35,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert4[] = {
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_NATU,
    },
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alex[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_NATU,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beck[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_STARAPTOR,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_PIDGEOT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Yasu[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_GLISCOR,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Takashi[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_WEEZING,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Dianne[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_LIGHT_SCREEN, MOVE_SHADOW_BALL},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_SCALD, MOVE_ICE_BEAM, MOVE_AMNESIA},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GLACEON,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_ICE_BEAM, MOVE_CALM_MIND, MOVE_MUD_SHOT, MOVE_PSYCHIC},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_HEAT_WAVE, MOVE_DARK_PULSE, MOVE_THUNDER_FANG, MOVE_SOLAR_BEAM},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_SHADOW_SNEAK, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_DRAPION,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_NIGHT_SLASH, MOVE_POISON_JAB, MOVE_X_SCISSOR, MOVE_ICE_FANG},
    .abilityNums = 1,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jani[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_BLISSEY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao1[] = {
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_SLUDGE, MOVE_ROCK_TOMB, MOVE_SMOKESCREEN, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_STUNKY,
    .moves = {MOVE_TOXIC, MOVE_POISON_FANG, MOVE_BITE, MOVE_HEADBUTT}
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_FANG, MOVE_POISON_TAIL, MOVE_BODY_SLAM, MOVE_GIGA_DRAIN}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lung[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_CROAGUNK,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_SKORUPI,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao2[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao3[] = {
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao4[] = {
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Lao5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE},
    .abilityNums = 0
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT},
    .abilityNums = 0
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT},
    .abilityNums = 0
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_SMOKE_BALL,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jocelyn[] = {
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_MANKEY,
    },
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_TYROGUE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Laura[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_RIOLU,
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MANKEY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy1[] = {
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_MAROWAK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cora[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paula[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy2[] = {
    {
    .iv = 110,
    .lvl = 36,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 110,
    .lvl = 36,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline1[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_ROSERADE,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clarissa[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_BELLOSSOM,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Angelica[] = {
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_WEATHER_BALL, MOVE_ICE_BEAM, MOVE_THUNDER, MOVE_ENERGY_BALL}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_LUDICOLO,
    .moves = {MOVE_ENERGY_BALL, MOVE_WEATHER_BALL, MOVE_ICE_BEAM, MOVE_LEECH_SEED}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline2[] = {
    {
    .iv = 10,
    .lvl = 29,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TACKLE, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline3[] = {
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_LEECH_SEED, MOVE_MEGA_DRAIN, MOVE_GRASS_WHISTLE, MOVE_SUNNY_DAY}
    },
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_FLAMETHROWER, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_SOLAR_BEAM, MOVE_SUNNY_DAY}
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_FLAMETHROWER, MOVE_TAKE_DOWN, MOVE_EARTHQUAKE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beverly[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GASTRODON_W,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Imani[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GOREBYSS,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_DEWGONG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kyla[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Denise[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_KINGLER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beth[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tara[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Missy[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_SLOWBRO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alice[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny1[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Grace[] = {
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 255,
    .lvl = 62,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tanya[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_VAPOREON,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_OCTILLERY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sharon[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nikki[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brenda[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GASTRODON_E,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GASTRODON_W,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katie[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_OCTILLERY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Susie[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GOREBYSS,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_LAPRAS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kara[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dana[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sienna[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_GOREBYSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Debra[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Linda[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_HORSEA,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kaylee[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Laurel[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_VAPOREON,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GASTRODON_W,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carlee[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny2[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny3[] = {
    {
    .iv = 0,
    .lvl = 41,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny4[] = {
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny5[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Heidi[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_ROCK_SLIDE, MOVE_SLASH}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CACTURNE,
    .moves = {MOVE_NEEDLE_ARM, MOVE_SHADOW_CLAW, MOVE_BULLDOZE, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Becky[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_GASTRODON_E,
    .moves = {MOVE_EARTH_POWER, MOVE_SURF, MOVE_CURSE, MOVE_RECOVER}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_DUGTRIO,
    .moves = {MOVE_EARTHQUAKE, MOVE_NIGHT_SLASH, MOVE_SLASH, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carol[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_PACHIRISU,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_NIDORINA,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_DRATINI,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nancy[] = {
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_XATU,
    },
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_LOPUNNY,
    },
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_VILEPLUME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Martha[] = {
    {
    .iv = 0,
    .lvl = 23,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 0,
    .lvl = 23,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana1[] = {
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_CLEFABLE,
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_GRANBULL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Cedric[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_DRIFBLIM,
    .moves = {MOVE_SHADOW_BALL, MOVE_AIR_SLASH, MOVE_PROTECT, MOVE_WILL_O_WISP}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Irene[] = {
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_DELIBIRD,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_CHINGLING,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_PHANPY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana2[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv1[] = {
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_MINUN,
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GinaAndMia1[] = {
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_CLEFFA,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_IGGLYBUFF,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_SEEDOT,
    },
    {
    .iv = 75,
    .lvl = 11,
    .species = SPECIES_LOTAD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MiuAndYuki[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_CLEFABLE,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_WIGGLYTUFF,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_LEAFEON,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_GLACEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv3[] = {
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_GinaAndMia2[] = {
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_DUSKULL,
    .moves = {MOVE_NIGHT_SHADE, MOVE_DISABLE, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_SHROOMISH,
    .moves = {MOVE_ABSORB, MOVE_LEECH_SEED, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv4[] = {
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AmyAndLiv5[] = {
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_MINUN,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AmyAndLiv6[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_MINUN,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Huey[] = {
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_SLOWPOKE,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_CORPHISH,
    },
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edmond[] = {
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_MANTYKE,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_HIPPOPOTAS,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_CORSOLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest1[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dwayne[] = {
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_BUIZEL,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_PRINPLUP,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Phillip[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leonard[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_FLOATZEL,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Duncan[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_GLALIE,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_TOXICROAK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest2[] = {
    {
    .iv = 10,
    .lvl = 36,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 36,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 10,
    .lvl = 36,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest4[] = {
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Eli[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_STEELIX,
    .moves = {MOVE_FIRE_FANG, MOVE_THUNDER_FANG, MOVE_IRON_HEAD, MOVE_EARTHQUAKE},
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_FIRE_PUNCH, MOVE_THUNDER_PUNCH, MOVE_BODY_SLAM, MOVE_BRICK_BREAK},
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_ICE_FANG, MOVE_FACADE},
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_DRAPION,
    .moves = {MOVE_FIRE_FANG, MOVE_NIGHT_SLASH, MOVE_POISON_JAB, MOVE_ICE_FANG},
    }
};

static const struct TrainerMonItemCustomMoves sParty_Annika[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ICE_BEAM, MOVE_SURF, MOVE_MOONBLAST, MOVE_RECOVER},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_FROSLASS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_ICE_BEAM, MOVE_SHADOW_BALL, MOVE_DRAINING_KISS, MOVE_DESTINY_BOND},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ICE_BEAM, MOVE_GIGA_DRAIN, MOVE_SURF, MOVE_LEECH_SEED},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jazmyn[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_ABSOL,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jonas[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_GUNK_SHOT, MOVE_ROCK_SLIDE, MOVE_SELF_DESTRUCT, MOVE_DARK_PULSE}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_MUK,
    .moves = {MOVE_GUNK_SHOT, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_KNOCK_OFF}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Kayley[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_SUNNY_DAY, MOVE_WEATHER_BALL, MOVE_FLAMETHROWER, MOVE_SOLAR_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_RAIN_DANCE, MOVE_WEATHER_BALL, MOVE_HYDRO_PUMP, MOVE_THUNDER}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_HAIL, MOVE_WEATHER_BALL, MOVE_BLIZZARD, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Auron[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_GLISCOR,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kelvin[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Marley[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER, MOVE_FLAMETHROWER, MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Reyna[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LUCARIO,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hudson[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Conor[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin1[] = {
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_STANTLER,
    },
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_SEVIPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hector[] = {
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_SEVIPER,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_ELECTRODE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_TabithaMossdeep[] = {
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_OUTRAGE, MOVE_IRON_HEAD, MOVE_SUNNY_DAY, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_SHIFTRY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH, MOVE_ROCK_SLIDE, MOVE_EXTRASENSORY},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HEAT_WAVE, MOVE_SOLAR_BEAM, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin2[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin3[] = {
    {
    .iv = 0,
    .lvl = 29,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 29,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin4[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin5[] = {
    {
    .iv = 0,
    .lvl = 35,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 0,
    .lvl = 35,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyVR1[] = {
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_QUICK_ATTACK, MOVE_DOUBLE_EDGE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_SLUDGE_BOMB, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_LIGHT_SCREEN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRoute103Mudkip[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute110Mudkip[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_AERIAL_ACE, MOVE_NIGHT_SLASH, MOVE_MUD_SLAP, MOVE_HEX},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_ICE_PUNCH, MOVE_TAKE_DOWN},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_FANG, MOVE_BULLDOZE, MOVE_HORN_ATTACK, MOVE_DOUBLE_KICK},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_PRINPLUP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_AURORA_BEAM, MOVE_AERIAL_ACE, MOVE_METAL_CLAW},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_QUILAVA,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_BULLDOZE, MOVE_SECRET_POWER, MOVE_QUICK_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_GROVYLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_RAZOR_LEAF, MOVE_DOUBLE_KICK, MOVE_DRAGON_BREATH, MOVE_ROCK_TOMB},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute119Mudkip[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_DRILL_PECK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_GIGA_IMPACT},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_SURF, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_HEAT_WAVE, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LEAF_BLADE, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRoute103Treecko[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_TORCHIC,
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute110Treecko[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_AERIAL_ACE, MOVE_NIGHT_SLASH, MOVE_MUD_SLAP, MOVE_HEX},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_ICE_PUNCH, MOVE_TAKE_DOWN},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_FANG, MOVE_BULLDOZE, MOVE_HORN_ATTACK, MOVE_DOUBLE_KICK},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_PRINPLUP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_AURORA_BEAM, MOVE_AERIAL_ACE, MOVE_METAL_CLAW},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_IVYSAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ACID, MOVE_LEECH_SEED, MOVE_SECRET_POWER},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_COMBUSKEN,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FIRE_PUNCH, MOVE_DOUBLE_KICK, MOVE_ROCK_TOMB, MOVE_QUICK_ATTACK},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute119Treecko[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_DRILL_PECK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_ICE_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_SURF, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_BLAZE_KICK, MOVE_CLOSE_COMBAT, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRoute103Torchic[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute110Torchic[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_AERIAL_ACE, MOVE_NIGHT_SLASH, MOVE_MUD_SLAP, MOVE_HEX},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_ICE_PUNCH, MOVE_TAKE_DOWN},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_FANG, MOVE_BULLDOZE, MOVE_HORN_ATTACK, MOVE_DOUBLE_KICK},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_IVYSAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ACID, MOVE_LEECH_SEED, MOVE_SECRET_POWER},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_QUILAVA,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_BULLDOZE, MOVE_SECRET_POWER, MOVE_QUICK_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_MARSHTOMP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_WATER_PULSE, MOVE_BULLDOZE, MOVE_ROCK_SLIDE, MOVE_ICE_PUNCH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRoute119Torchic[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_DRILL_PECK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_HEAT_WAVE, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_AQUA_JET},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute103Mudkip[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute110Mudkip[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_AIR_CUTTER, MOVE_QUICK_ATTACK, MOVE_TWISTER, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_HAUNTER,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_SLUDGE, MOVE_WILL_O_WISP, MOVE_HEX},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SLUDGE, MOVE_BULLDOZE, MOVE_BITE, MOVE_DOUBLE_KICK},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_CROCONAW,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_BITE, MOVE_SLASH, MOVE_ICE_FANG},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MONFERNO,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_MACH_PUNCH, MOVE_FLAME_WHEEL, MOVE_ROCK_TOMB, MOVE_SLACK_OFF},
    .abilityNums = 1,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_GROVYLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_DOUBLE_KICK, MOVE_MEGA_DRAIN, MOVE_DRAGON_BREATH, MOVE_ROCK_TOMB},
    .abilityNums = 1,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute119Mudkip[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_UPROAR, MOVE_RAZOR_WIND, MOVE_HEAT_WAVE, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_SUPERPOWER, MOVE_CRUNCH},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_CRUNCH, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH},
    .abilityNums = 0,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_ENERGY_BALL, MOVE_DRAGON_PULSE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute103Treecko[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_TORCHIC,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute110Treecko[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_AIR_CUTTER, MOVE_QUICK_ATTACK, MOVE_TWISTER, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_HAUNTER,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_SLUDGE, MOVE_WILL_O_WISP, MOVE_HEX},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SLUDGE, MOVE_BULLDOZE, MOVE_BITE, MOVE_DOUBLE_KICK},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_GROTLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_RAZOR_LEAF, MOVE_BITE, MOVE_BULLDOZE, MOVE_CURSE},
    .abilityNums = 1,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_CROCONAW,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_BITE, MOVE_SLASH, MOVE_ICE_FANG},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_COMBUSKEN,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FIRE_PUNCH, MOVE_DOUBLE_KICK, MOVE_ROCK_TOMB, MOVE_QUICK_ATTACK},
    .abilityNums = 1,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute119Treecko[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_UPROAR, MOVE_RAZOR_WIND, MOVE_HEAT_WAVE, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_SUPERPOWER, MOVE_CRUNCH},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_SEED_BOMB, MOVE_BODY_PRESS},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_CRUNCH, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_BLAZE_KICK, MOVE_CLOSE_COMBAT, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 1,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute103Torchic[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute110Torchic[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_AIR_CUTTER, MOVE_QUICK_ATTACK, MOVE_TWISTER, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_HAUNTER,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_SLUDGE, MOVE_WILL_O_WISP, MOVE_HEX},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_NIDORINA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SLUDGE, MOVE_BULLDOZE, MOVE_BITE, MOVE_DOUBLE_KICK},
    .abilityNums = 0,
    },
    {
    .iv = 100,
    .lvl = 25,
    .species = SPECIES_GROTLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_RAZOR_LEAF, MOVE_BITE, MOVE_BULLDOZE, MOVE_CURSE},
    .abilityNums = 1,
    },
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MONFERNO,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_MACH_PUNCH, MOVE_FLAME_WHEEL, MOVE_ROCK_TOMB, MOVE_SLACK_OFF},
    .abilityNums = 1,
    },
    {
    .iv = 100,
    .lvl = 25,
    .species = SPECIES_MARSHTOMP,
    .heldItem = ITEM_SOFT_SAND,
    .moves = {MOVE_WATER_PULSE, MOVE_BULLDOZE, MOVE_ROCK_SLIDE, MOVE_ICE_PUNCH},
    .abilityNums = 0,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute119Torchic[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_UPROAR, MOVE_RAZOR_WIND, MOVE_HEAT_WAVE, MOVE_STEEL_WING},
    .abilityNums = 0,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_SUPERPOWER, MOVE_CRUNCH},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_SEED_BOMB, MOVE_BODY_PRESS},
    .abilityNums = 1,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_AQUA_JET},
    .abilityNums = 0,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac1[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MIME_JR,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_RIOLU,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MUNCHLAX,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_PORYGON,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_BONSLY,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_PLUSLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Davis[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_HERACROSS,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Mitchell[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_POWER_GEM, MOVE_SHADOW_BALL, MOVE_MOONBLAST},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_FLARE_BLITZ, MOVE_SOLAR_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_SEED_BOMB, MOVE_ROCK_SLIDE, MOVE_ZEN_HEADBUTT},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SLUDGE_WAVE, MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_GIGA_DRAIN},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_BANETTE,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_STRIKE, MOVE_SHADOW_SNEAK, MOVE_NIGHT_SLASH, MOVE_GUNK_SHOT},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_DRACO_METEOR, MOVE_MOONBLAST, MOVE_HEAT_WAVE, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_ARON,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_ARON,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_ARON,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac5[] = {
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia1[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_IGGLYBUFF,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_CLEFFA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_PICHU,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_BUDEW,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_HAPPINY,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Halle[] = {
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SABLEYE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FAKE_OUT, MOVE_SHADOW_BALL, MOVE_SHADOW_SNEAK, MOVE_ANCIENT_POWER},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_AQUA_TAIL, MOVE_AQUA_JET, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HEAT_WAVE, MOVE_DRAGON_PULSE, MOVE_EARTHQUAKE, MOVE_SOLAR_BEAM},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_POWER_GEM, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_BELLOSSOM,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_ENERGY_BALL, MOVE_MOONBLAST, MOVE_PSYCHIC, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Garrison[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia5[] = {
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson1[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_SLOWBRO,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_UMBREON,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_GALLADE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lorenzo[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sebastian[] = {
    {
    .iv = 50,
    .lvl = 39,
    .species = SPECIES_CACTURNE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson2[] = {
    {
    .iv = 60,
    .lvl = 31,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson3[] = {
    {
    .iv = 70,
    .lvl = 34,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson4[] = {
    {
    .iv = 80,
    .lvl = 37,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson5[] = {
    {
    .iv = 90,
    .lvl = 39,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = 90,
    .lvl = 39,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine1[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_ESPEON,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenna[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_FLOATZEL,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sophia[] = {
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine2[] = {
    {
    .iv = 60,
    .lvl = 30,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 60,
    .lvl = 30,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine3[] = {
    {
    .iv = 70,
    .lvl = 33,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 70,
    .lvl = 33,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine4[] = {
    {
    .iv = 80,
    .lvl = 36,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 80,
    .lvl = 36,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine5[] = {
    {
    .iv = 90,
    .lvl = 39,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 90,
    .lvl = 39,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Julio[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_SNEASEL,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_CARNIVINE,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern5[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_CROBAT
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_CARNIVINE,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HITMONTOP,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntUnused[] = {
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre4[] = {
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_CRAWDAUNT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntJaggedPass[] = {
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Marc[] = {
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_ROCK_THROW, MOVE_SLUDGE, MOVE_ROCK_TOMB, MOVE_POISON_GAS}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_BONSLY,
    .moves = {MOVE_ROCK_TOMB, MOVE_LOW_KICK, MOVE_VINE_WHIP, MOVE_ROCK_THROW}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_ONIX,
    .moves = {MOVE_DRAGON_BREATH, MOVE_ROCK_TOMB, MOVE_BULLDOZE, MOVE_RAGE}
    },
    {
    .iv = 100,
    .lvl = 14,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_PSYBEAM, MOVE_ANCIENT_POWER, MOVE_MUD_SLAP, MOVE_RAPID_SPIN}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brenden[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_CROAGUNK,
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lilith[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_TYROGUE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristian[] = {
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sylvia[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_JYNX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leonardo[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Athena[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_LOPUNNY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_FAKE_OUT, MOVE_HI_JUMP_KICK, MOVE_ICE_PUNCH, MOVE_STRENGTH}
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_DEWGONG,
    .heldItem = ITEM_NEVER_MELT_ICE,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_DRILL_RUN, MOVE_PLAY_ROUGH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Harrison[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtChimney2[] = {
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_SKUNTANK,
    },
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_MAROWAK,
    },
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_RAPIDASH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clarence[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Terry[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nate[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_ESPEON,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_MR_MIME,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_CHIMECHO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kathleen[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GRUMPIG,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_ESPEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clifford[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GIRAFARIG,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_SLOWBRO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicholas[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_WOBBUFFET,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_GALLADE,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_BRONZONG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter3[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CACTURNE,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter4[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SNORLAX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter5[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_TORKOAL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter6[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_VILEPLUME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter7[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_LUXRAY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Macey[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_XATU,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRustboroTreecko[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_WING_ATTACK, MOVE_PURSUIT, MOVE_MUD_SLAP, MOVE_ASTONISH},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_THUNDER_WAVE, MOVE_COTTON_SPORE, MOVE_HEADBUTT},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_PIPLUP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_WATER_PULSE, MOVE_ICY_WIND, MOVE_CHARM, MOVE_PECK},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_BULBASAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ACID, MOVE_LEECH_SEED, MOVE_SECRET_POWER},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_COMBUSKEN,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_DOUBLE_KICK, MOVE_ROCK_TOMB, MOVE_QUICK_ATTACK},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRustboroMudkip[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_WING_ATTACK, MOVE_PURSUIT, MOVE_MUD_SLAP, MOVE_ASTONISH},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_THUNDER_WAVE, MOVE_COTTON_SPORE, MOVE_HEADBUTT},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_PIPLUP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_WATER_PULSE, MOVE_ICY_WIND, MOVE_CHARM, MOVE_PECK},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_CYNDAQUIL,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_ROLLOUT, MOVE_QUICK_ATTACK, MOVE_SECRET_POWER},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_GROVYLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_DRAGON_BREATH, MOVE_ROCK_TOMB, MOVE_QUICK_ATTACK},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paxton[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_STARAPTOR,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isabella[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst5[] = {
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_GOREBYSS,
    },
    {
    .iv = 200,
    .lvl = 48,
    .species = SPECIES_PURUGLY,
    }
};

static const struct TrainerMonItemCustomMoves sParty_TabithaMtChimney[] = {
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_CROBAT,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_CRUNCH, MOVE_POISON_FANG, MOVE_GIGA_DRAIN, MOVE_WING_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CRUNCH, MOVE_SHOCK_WAVE, MOVE_ICE_FANG, MOVE_FIRE_FANG},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_CURSE, MOVE_BODY_SLAM, MOVE_OVERHEAT, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_MUK,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_BULLDOZE, MOVE_POISON_JAB, MOVE_KNOCK_OFF, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BODY_SLAM, MOVE_DIZZY_PUNCH, MOVE_CRUNCH, MOVE_ROCK_SLIDE},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_WILL_O_WISP, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jonathan[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_YANMEGA,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MAGNEZONE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanRustboroTorchic[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_WING_ATTACK, MOVE_PURSUIT, MOVE_MUD_SLAP, MOVE_ASTONISH},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_THUNDER_WAVE, MOVE_COTTON_SPORE, MOVE_HEADBUTT},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_BULBASAUR,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_ACID, MOVE_LEECH_SEED, MOVE_SECRET_POWER},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_CYNDAQUIL,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_ROLLOUT, MOVE_QUICK_ATTACK, MOVE_SECRET_POWER},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_MARSHTOMP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BULLDOZE, MOVE_ROCK_TOMB, MOVE_WATER_PULSE, MOVE_ICY_WIND},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRustboroMudkip[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_PIDGEY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_SWIFT, MOVE_QUICK_ATTACK, MOVE_GUST, MOVE_AIR_CUTTER},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_GASTLY,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_ACID, MOVE_SHADOW_SNEAK, MOVE_HYPNOSIS, MOVE_NIGHT_SHADE},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_TOTODILE,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BITE, MOVE_AQUA_JET, MOVE_WATER_GUN, MOVE_SCRATCH},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_CHIMCHAR,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_SEISMIC_TOSS, MOVE_ROCK_TOMB, MOVE_SCRATCH},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_GROVYLE,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_MEGA_DRAIN, MOVE_DRAGON_BREATH, MOVE_CUT, MOVE_FAINT_ATTACK},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_MaxieMagmaHideout[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SUNNY_DAY, MOVE_BRICK_BREAK, MOVE_SHADOW_SNEAK, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_THUNDER_PUNCH, MOVE_IRON_TAIL},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HEAT_WAVE, MOVE_SOLAR_BEAM, MOVE_DRAGON_PULSE, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_EXEGGUTOR,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_GIGA_DRAIN, MOVE_PSYCHIC, MOVE_SOLAR_BEAM, MOVE_SLUDGE_BOMB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_SOLAR_BEAM, MOVE_SLUDGE_BOMB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_ICE_PUNCH, MOVE_CRUNCH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_MaxieMtChimney[] = {
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_SUNNY_DAY, MOVE_DUAL_WINGBEAT, MOVE_FACADE},
    .abilityNums = 0
    },
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_RHYDON,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_DRILL_RUN, MOVE_ROCK_SLIDE, MOVE_ICE_FANG, MOVE_THUNDER_FANG},
    .abilityNums = 0
    },
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_DUSCLOPS,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WILL_O_WISP, MOVE_SHADOW_PUNCH, MOVE_SEISMIC_TOSS, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_EXEGGUTOR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_PSYCHIC, MOVE_SLUDGE_BOMB, MOVE_STOMP},
    .abilityNums = 0
    },
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_POISON_JAB, MOVE_CROSS_CHOP, MOVE_KNOCK_OFF, MOVE_DIG},
    .abilityNums = 1
    },
    {
    .iv = 180,
    .lvl = 38,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EXTREME_SPEED, MOVE_FIRE_FANG, MOVE_THUNDER_FANG, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tiana[] = {
    {
    .iv = 25,
    .lvl = 6,
    .species = SPECIES_PIDGEY,
    },
    {
    .iv = 25,
    .lvl = 6,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 25,
    .lvl = 6,
    .species = SPECIES_EEVEE,
    },
    {
    .iv = 25,
    .lvl = 6,
    .species = SPECIES_BUDEW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley1[] = {
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_NIDORAN_F,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_GLAMEOW,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_NATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Janice[] = {
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_SLAKOTH,
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Vivi[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_SMOOCHUM,
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_LEDIAN,
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_CORSOLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sally[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_CACNEA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_SKIPLOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robin[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_EKANS,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_NIDORINA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_NIDORINO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andrea[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GLALIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Crissy[] = {
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rick[] = {
    {
    .iv = 50,
    .lvl = 6,
    .species = SPECIES_BURMY_S,
    },
    {
    .iv = 50,
    .lvl = 6,
    .species = SPECIES_WEEDLE,
    },
    {
    .iv = 50,
    .lvl = 6,
    .species = SPECIES_SPINARAK,
    },
    {
    .iv = 50,
    .lvl = 6,
    .species = SPECIES_CATERPIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lyle[] = {
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_CASCOON,
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_SILCOON,
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_NINCADA,
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_KRICKETOT,
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_LEDYBA,
    },
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_YANMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jose[] = {
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_COMBEE,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_LEDYBA,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_PARAS,
    },
    {
    .iv = 75,
    .lvl = 12,
    .species = SPECIES_BURMY_P,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Doug[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_VENOMOTH,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_SHUCKLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Greg[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kent[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_DRAPION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James1[] = {
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_PARAS,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_METAPOD,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_KAKUNA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brice[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_LARVITAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent1[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_RHYHORN,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_RELICANTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lenny[] = {
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_SUDOWOODO,
    },
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 150,
    .lvl = 35,
    .species = SPECIES_GABITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lucas1[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_ONIX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alan[] = {
    {
    .iv = 0,
    .lvl = 22,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 22,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 0,
    .lvl = 22,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clark[] = {
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_BRONZOR,
    },
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_BARBOACH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Eric[] = {
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_RHYDON,
    },
    {
    .iv = 100,
    .lvl = 37,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lucas2[] = {
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_SPLASH, MOVE_WATER_GUN, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Mike1[] = {
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_GUST, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_POOCHYENA,
    .moves = {MOVE_BITE, MOVE_SCARY_FACE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mike2[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_TRAPINCH,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_KOFFING,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent2[] = {
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent3[] = {
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 20,
    .lvl = 27,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent4[] = {
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GOLEM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_DezAndLuke[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_ARCANINE
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LeaAndJed[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CHARIZARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan1[] = {
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan5[] = {
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Johanna[] = {
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 80,
    .lvl = 18,
    .species = SPECIES_KRABBY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Gerald[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_FLAREON,
    .moves = {MOVE_FIRE_FANG, MOVE_BODY_SLAM, MOVE_CRUNCH, MOVE_DOUBLE_KICK}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_FLAMETHROWER, MOVE_EARTH_POWER, MOVE_BODY_SLAM, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_FIRE_PUNCH, MOVE_BODY_SLAM, MOVE_FAKE_OUT, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Vivian[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_VOLTORB,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_ROLLOUT}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_PIKACHU,
    .moves = {MOVE_SHOCK_WAVE, MOVE_HEADBUTT, MOVE_NUZZLE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_CHINCHOU,
    .moves = {MOVE_SHOCK_WAVE, MOVE_BUBBLE_BEAM, MOVE_THUNDER_WAVE, MOVE_WATER_PULSE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Danielle[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH, MOVE_ZEN_HEADBUTT}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CLEFABLE,
    .moves = {MOVE_FLAMETHROWER, MOVE_PSYCHIC, MOVE_TRI_ATTACK, MOVE_RECOVER}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_PIDGEOT,
    .moves = {MOVE_HEAT_WAVE, MOVE_RAZOR_WIND, MOVE_UPROAR, MOVE_TWISTER}
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_NIDOQUEEN,
    .moves = {MOVE_FLAMETHROWER, MOVE_ICE_PUNCH, MOVE_POISON_JAB, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Hideo[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH, MOVE_RAZOR_WIND, MOVE_EXTRASENSORY}
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_SCYTHER,
    .moves = {MOVE_X_SCISSOR, MOVE_DUAL_WINGBEAT, MOVE_LEAF_BLADE, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Keigo[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_GUNK_SHOT, MOVE_ROCK_SLIDE, MOVE_SELF_DESTRUCT, MOVE_DARK_PULSE}
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_UMBREON,
    .moves = {MOVE_CURSE, MOVE_FOUL_PLAY, MOVE_TOXIC, MOVE_MOONLIGHT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Riley[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_CROBAT,
    .moves = {MOVE_GUNK_SHOT, MOVE_DUAL_WINGBEAT, MOVE_STEEL_WING, MOVE_GIGA_DRAIN}
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_MUK,
    .moves = {MOVE_GUNK_SHOT, MOVE_EARTHQUAKE, MOVE_KNOCK_OFF, MOVE_SHADOW_SNEAK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Flint[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_HONCHKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ashley[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_XATU,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyMauville[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_STARAVIA,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_QUICK_ATTACK, MOVE_AERIAL_ACE, MOVE_FACADE, MOVE_WING_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_GABITE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BULLDOZE, MOVE_BITE, MOVE_DRAGON_BREATH, MOVE_METAL_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_SLUDGE, MOVE_MEGA_DRAIN, MOVE_EXTRASENSORY, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SHOCK_WAVE, MOVE_THUNDER_WAVE, MOVE_TRI_ATTACK, MOVE_SCREECH},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 24,
    .species = SPECIES_KIRLIA,
    .heldItem = ITEM_BERRY_JUICE,
    .moves = {MOVE_PSYBEAM, MOVE_DRAINING_KISS, MOVE_MAGICAL_LEAF, MOVE_ICY_WIND},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyVR2[] = {
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_QUICK_ATTACK, MOVE_DOUBLE_EDGE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_LIGHT_SCREEN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyVR3[] = {
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_QUICK_ATTACK, MOVE_DOUBLE_EDGE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_LIGHT_SCREEN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 91,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyVR4[] = {
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_QUICK_ATTACK, MOVE_DOUBLE_EDGE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_LIGHT_SCREEN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 93,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_WallyVR5[] = {
    {
    .iv = 255,
    .lvl = 95,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_QUICK_ATTACK, MOVE_DOUBLE_EDGE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 95,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SHADOW_BALL, MOVE_ENERGY_BALL, MOVE_SLUDGE_WAVE, MOVE_DAZZLING_GLEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 95,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_PLAY_ROUGH, MOVE_ICE_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 95,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 95,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_LIGHT_SCREEN},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 97,
    .species = SPECIES_GALLADE,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_PSYCHO_CUT, MOVE_CLOSE_COMBAT, MOVE_LEAF_BLADE, MOVE_NIGHT_SLASH},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveMudkip[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_SKY_ATTACK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRACO_METEOR, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_PSYCHIC_FANGS, MOVE_HAMMER_ARM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_HYDRO_PUMP, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_ERUPTION, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LEAF_BLADE, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveTreecko[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_SKY_ATTACK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRACO_METEOR, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_PSYCHIC_FANGS, MOVE_HAMMER_ARM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_HYDRO_PUMP, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_BLAZE_KICK, MOVE_CLOSE_COMBAT, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveTorchic[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_SKY_ATTACK, MOVE_NIGHT_SLASH, MOVE_STEEL_WING, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRACO_METEOR, MOVE_ICE_PUNCH, MOVE_POWER_GEM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_PSYCHIC_FANGS, MOVE_HAMMER_ARM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_EARTH_POWER, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_ERUPTION, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_AQUA_JET},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayLilycoveMudkip[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AIR_SLASH, MOVE_MOONBLAST, MOVE_AURA_SPHERE, MOVE_ANCIENT_POWER},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_PORYGON_Z,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_CRUNCH, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_SCEPTILE,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_ENERGY_BALL, MOVE_DRAGON_PULSE, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayLilycoveTreecko[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AIR_SLASH, MOVE_MOONBLAST, MOVE_AURA_SPHERE, MOVE_ANCIENT_POWER},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_PORYGON_Z,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_SEED_BOMB, MOVE_BODY_PRESS},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AQUA_TAIL, MOVE_CRUNCH, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH},
    .abilityNums = 0,
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_BLAZE_KICK, MOVE_CLOSE_COMBAT, MOVE_THUNDER_PUNCH, MOVE_SHADOW_CLAW},
    .abilityNums = 1,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayLilycoveTorchic[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TOGEKISS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_AIR_SLASH, MOVE_MOONBLAST, MOVE_AURA_SPHERE, MOVE_ANCIENT_POWER},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_BOMB, MOVE_NASTY_PLOT, MOVE_GIGA_DRAIN},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_PORYGON_Z,
    .heldItem = ITEM_CHOICE_SPECS,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_PSYCHIC},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_TORTERRA,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_EARTHQUAKE, MOVE_ROCK_SLIDE, MOVE_SEED_BOMB, MOVE_BODY_PRESS},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_THUNDER_PUNCH, MOVE_EARTHQUAKE},
    .abilityNums = 1,
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_LIQUIDATION, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_AQUA_JET},
    .abilityNums = 0,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jonah[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_RELICANTH,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_HUNTAIL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Henry[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Roger[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_QWILFISH,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_SLOWBRO,
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alexa[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ruben[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_PROBOPASS,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_YANMEGA,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_MAGMORTAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji1[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_LUCARIO,
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wayne[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_OMASTAR,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_QUAGSIRE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Aidan[] = {
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Reed[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_GOLDUCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tisha[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_MILOTIC,
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_VAPOREON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ToriAndTia[] = {
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_GASTRODON_E,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_GASTRODON_W,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_KimAndIris[] = {
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_TOGEKISS,
    .moves = {MOVE_MOONBLAST, MOVE_AIR_SLASH, MOVE_EXTREME_SPEED, MOVE_FOLLOW_ME}
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_GLACEON,
    .moves = {MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_ICE_SHARD}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_TyraAndIvy[] = {
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_LUDICOLO,
    .moves = {MOVE_WATER_PULSE, MOVE_GIGA_DRAIN, MOVE_FAKE_OUT, MOVE_KNOCK_OFF}
    },
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_FAINT_ATTACK, MOVE_RAZOR_LEAF, MOVE_HEADBUTT, MOVE_FAKE_OUT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_MelAndPaul[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_LAPRAS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_PROTECT}
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_AMPHAROS,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_FIRE_PUNCH, MOVE_POWER_GEM}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay1[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_SHADOW_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH, MOVE_IRON_HEAD}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay2[] = {
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_SHADOW_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 75,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH, MOVE_IRON_HEAD}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay3[] = {
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_SHADOW_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH, MOVE_IRON_HEAD}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay4[] = {
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_SHADOW_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH, MOVE_IRON_HEAD}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay5[] = {
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_SHADOW_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_PUNCH, MOVE_IRON_HEAD}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ReliAndIan[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_GOREBYSS,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_HUNTAIL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy1[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy2[] = {
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 0,
    .lvl = 40,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy3[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy4[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy5[] = {
    {
    .iv = 0,
    .lvl = 51,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LisaAndRay[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chris[] = {
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 200,
    .lvl = 46,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Dawson[] = {
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_TRAPINCH,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_ELECTRIKE,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_AIPOM,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Sarah[] = {
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_MAREEP,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_SWINUB,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 75,
    .lvl = 14,
    .species = SPECIES_CHINGLING,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darian[] = {
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_AZURILL,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_POLIWAG,
    },
    {
    .iv = 50,
    .lvl = 7,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hailey[] = {
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_SHELLOS_W,
    },
    {
    .iv = 75,
    .lvl = 17,
    .species = SPECIES_HAPPINY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chandler[] = {
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_SPHEAL,
    },
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 75,
    .lvl = 18,
    .species = SPECIES_WOOPER,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Kaleb[] = {
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_MUNCHLAX,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_WYNAUT,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 80,
    .lvl = 19,
    .species = SPECIES_DELIBIRD,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Joseph[] = {
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_ELECTABUZZ,
    },
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_VOLTORB,
    },
    {
    .iv = 80,
    .lvl = 21,
    .species = SPECIES_LUXIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alyssa[] = {
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_PACHIRISU,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_SNEASEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marcos[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_FLAAFFY,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_LUXIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rhett[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_HITMONCHAN,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_HITMONLEE,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_HITMONTOP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tyron[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_GLIGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Celina[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_SUNFLORA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bianca[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_EXEGGCUTE,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_WEEPINBELL,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_WORMADAM_P,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hayden[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_CHARMELEON,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sophie[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_CHANSEY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Coby[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_FEAROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lawrence[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_STARAVIA,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wyatt[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_LICKITUNG,
    },
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Angelina[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_JYNX,
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_VICTREEBEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kai[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_OCTILLERY,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_MANTINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Charlotte[] = {
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_SHUCKLE,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_FLAAFFY,
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Deandre[] = {
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_TOGETIC,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_HIPPOPOTAS,
    },
    {
    .iv = 80,
    .lvl = 22,
    .species = SPECIES_NOCTOWL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout1[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_FLYGON,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_RHYDON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout2[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_ABSOL,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_TOXICROAK,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout3[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_RAPIDASH,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout4[] = {
    {
    .iv = 200,
    .lvl = 56,
    .species = SPECIES_CRADILY,
    },
    {
    .iv = 200,
    .lvl = 56,
    .species = SPECIES_SWALOT,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_AGGRON,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_MAGCARGO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout5[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_KECLEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout6[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_MAGMAR,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_PROBOPASS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout7[] = {
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_GIRAFARIG,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_CACTURNE,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout8[] = {
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_STARAPTOR,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_SUNFLORA,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_HOUNDOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout9[] = {
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 200,
    .lvl = 58,
    .species = SPECIES_DONPHAN,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_ARCANINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout10[] = {
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_FLAREON,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_SCIZOR,
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_MAMOSWINE,
    },
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_GLISCOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout11[] = {
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_ARMALDO,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_AMPHAROS,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_AERODACTYL,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_HOUNDOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout12[] = {
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_STEELIX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout13[] = {
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_VICTREEBEL,
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 200,
    .lvl = 62,
    .species = SPECIES_MAGMORTAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout14[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_LUXRAY,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_SEVIPER,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_VILEPLUME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout15[] = {
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 200,
    .lvl = 57,
    .species = SPECIES_GRANBULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout16[] = {
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_NIDOQUEEN,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 200,
    .lvl = 59,
    .species = SPECIES_MAGCARGO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_TabithaMagmaHideout[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_DODRIO,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_DRILL_PECK, MOVE_DRILL_RUN, MOVE_THRASH, MOVE_KNOCK_OFF},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_EARTH_POWER, MOVE_THUNDER_PUNCH, MOVE_SOLAR_BEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_BRICK_BREAK},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_SLUDGE_BOMB, MOVE_LEECH_SEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BODY_SLAM, MOVE_FAKE_OUT, MOVE_CRUNCH, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_OUTRAGE, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_FLAMETHROWER},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darcy[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_MILOTIC,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MaxieMossdeep[] = {
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_SOFT_SAND,
    .moves = {MOVE_EARTH_POWER, MOVE_OUTRAGE, MOVE_GIGA_DRAIN, MOVE_HEAT_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_ROCK_SLIDE, MOVE_ICE_PUNCH, MOVE_CRUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_HEAT_WAVE, MOVE_SOLAR_BEAM, MOVE_DRAGON_PULSE, MOVE_ANCIENT_POWER},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pete[] = {
    {
    .iv = 0,
    .lvl = 15,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isabelle[] = {
    {
    .iv = 0,
    .lvl = 15,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres1[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_DUGTRIO,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GABITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Josue[] = {
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 200,
    .lvl = 42,
    .species = SPECIES_GLISCOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Camron[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_POLITOED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory1[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_PRIMEAPE,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carolina[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_GENGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elijah[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Celia[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_QUAGSIRE,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CARNIVINE,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SUDOWOODO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossBrandon[] = {
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HEAD_SMASH, MOVE_DRAGON_CLAW, MOVE_IRON_HEAD, MOVE_WING_ATTACK},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_IRON_HEAD, MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_LEECH_SEED, MOVE_CURSE},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_SANDSLASH,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_SWORDS_DANCE, MOVE_POISON_JAB},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_MAMOSWINE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_ICICLE_CRASH, MOVE_ICE_SHARD, MOVE_BODY_PRESS},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_SLUDGE_WAVE, MOVE_ICE_BEAM, MOVE_EARTH_POWER},
    .abilityNums = 0
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Branden[] = {
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 200,
    .lvl = 40,
    .species = SPECIES_RAMPARDOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bryant[] = {
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_QUILAVA,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_PONYTA,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_MAGMAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shayla[] = {
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_IVYSAUR,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_CARNIVINE,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_JUMPLUFF,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kyra[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_PIDGEOT,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossLucy[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_SEVIPER,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_POISON_JAB, MOVE_CRUNCH, MOVE_GIGA_DRAIN, MOVE_TOXIC},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_MOONBLAST, MOVE_RECOVER},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_IRON_HEAD, MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_ICE_FANG},
    .abilityNums = 1
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_AQUA_TAIL, MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_DRAGON_DANCE},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_ARBOK,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_POISON_JAB, MOVE_SLAM, MOVE_ICE_FANG, MOVE_CRUNCH},
    .abilityNums = 0
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_SHUCKLE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_TOXIC, MOVE_SAND_TOMB, MOVE_PROTECT, MOVE_REST},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alix[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_ALAKAZAM,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_MR_MIME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Helene[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_LOPUNNY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marlene[] = {
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_GOLDUCK,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiniBossAnabel[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_SNEASEL,
    .heldItem = ITEM_SCOPE_LENS,
    .moves = {MOVE_FAKE_OUT, MOVE_ICE_SHARD, MOVE_KNOCK_OFF, MOVE_AERIAL_ACE},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MISDREAVUS,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DISARMING_VOICE, MOVE_HEX, MOVE_PSYBEAM, MOVE_WILL_O_WISP},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_RIOLU,
    .heldItem = ITEM_SALAC_BERRY,
    .moves = {MOVE_MACH_PUNCH, MOVE_QUICK_ATTACK, MOVE_BITE, MOVE_METAL_CLAW},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_BAGON,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_BITE, MOVE_EMBER, MOVE_DRAGON_BREATH, MOVE_HEADBUTT},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MUNCHLAX,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HEADBUTT, MOVE_BITE, MOVE_LICK, MOVE_BULLDOZE},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_KADABRA,
    .heldItem = ITEM_TWISTED_SPOON,
    .moves = {MOVE_PSYBEAM, MOVE_SIGNAL_BEAM, MOVE_KNOCK_OFF, MOVE_CONFUSION},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Johnson[] = {
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_HOUNDOUR,
    },
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_PHANPY,
    },
    {
    .iv = 75,
    .lvl = 13,
    .species = SPECIES_DRIFLOON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Melina[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_NOCTOWL,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_FARFETCHD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brandi[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_BALTOY,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_DELCATTY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Aisha[] = {
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_BUNEARY,
    },
    {
    .iv = 80,
    .lvl = 23,
    .species = SPECIES_CROAGUNK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Makayla[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_TOGEKISS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fabian[] = {
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 200,
    .lvl = 49,
    .species = SPECIES_LUXRAY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dayton[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_MAGMORTAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rachel[] = {
    {
    .iv = 200,
    .lvl = 47,
    .species = SPECIES_LAPRAS,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Leonel[] = {
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_THUNDER, MOVE_QUICK_ATTACK, MOVE_THUNDER_WAVE, MOVE_ICE_FANG}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_AQUA_JET, MOVE_LIQUIDATION, MOVE_ICE_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Callie[] = {
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 200,
    .lvl = 50,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cale[] = {
    {
    .iv = 200,
    .lvl = 53,
    .species = SPECIES_HERACROSS,
    },
    {
    .iv = 200,
    .lvl = 53,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Myles[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_NIDOKING,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_GLALIE,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_GALLADE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pat[] = {
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_NIDOQUEEN,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_FROSLASS,
    },
    {
    .iv = 200,
    .lvl = 54,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin1[] = {
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 200,
    .lvl = 55,
    .species = SPECIES_CRAWDAUNT,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRustboroTreecko[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_PIDGEY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_SWIFT, MOVE_QUICK_ATTACK, MOVE_GUST, MOVE_AIR_CUTTER},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_GASTLY,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_ACID, MOVE_SHADOW_SNEAK, MOVE_HYPNOSIS, MOVE_NIGHT_SHADE},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_TOTODILE,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BITE, MOVE_AQUA_JET, MOVE_WATER_GUN, MOVE_SCRATCH},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_TURTWIG,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_VINE_WHIP, MOVE_HEADBUTT, MOVE_RAZOR_LEAF, MOVE_BULLDOZE},
    .abilityNums = 1
    },
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_COMBUSKEN,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_DOUBLE_KICK, MOVE_QUICK_ATTACK, MOVE_SEISMIC_TOSS},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRustboroTorchic[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_PIDGEY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_SWIFT, MOVE_QUICK_ATTACK, MOVE_GUST, MOVE_AIR_CUTTER},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_GASTLY,
    .heldItem = ITEM_SPELL_TAG,
    .moves = {MOVE_ACID, MOVE_SHADOW_SNEAK, MOVE_HYPNOSIS, MOVE_NIGHT_SHADE},
    .abilityNums = 0
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_TURTWIG,
    .heldItem = ITEM_MIRACLE_SEED,
    .moves = {MOVE_VINE_WHIP, MOVE_HEADBUTT, MOVE_RAZOR_LEAF, MOVE_BULLDOZE},
    .abilityNums = 1
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_CHIMCHAR,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_FLAME_WHEEL, MOVE_SEISMIC_TOSS, MOVE_ROCK_TOMB, MOVE_SCRATCH},
    .abilityNums = 0
    },
    {
    .iv = 150,
    .lvl = 17,
    .species = SPECIES_MARSHTOMP,
    .heldItem = ITEM_MYSTIC_WATER,
    .moves = {MOVE_BULLDOZE, MOVE_ROCK_THROW, MOVE_WATER_PULSE, MOVE_PROTECT},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne2[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_PROBOPASS,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SANDSTORM, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_ROCK_SLIDE, MOVE_X_SCISSOR, MOVE_BRICK_BREAK, MOVE_SWORDS_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_TOXIC, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ROCK_SLIDE, MOVE_CRUNCH, MOVE_ICE_PUNCH, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SCALD, MOVE_ANCIENT_POWER, MOVE_ICE_BEAM, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_RAMPARDOS,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_THUNDER_PUNCH, MOVE_BRICK_BREAK},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne3[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_PROBOPASS,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SANDSTORM, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_ROCK_SLIDE, MOVE_X_SCISSOR, MOVE_BRICK_BREAK, MOVE_SWORDS_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_TOXIC, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ROCK_SLIDE, MOVE_CRUNCH, MOVE_ICE_PUNCH, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SCALD, MOVE_ANCIENT_POWER, MOVE_ICE_BEAM, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_RAMPARDOS,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_THUNDER_PUNCH, MOVE_BRICK_BREAK},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne4[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_PROBOPASS,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SANDSTORM, MOVE_FLASH_CANNON, MOVE_THUNDERBOLT, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_ROCK_SLIDE, MOVE_X_SCISSOR, MOVE_BRICK_BREAK, MOVE_SWORDS_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_TOXIC, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ROCK_SLIDE, MOVE_CRUNCH, MOVE_ICE_PUNCH, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SCALD, MOVE_ANCIENT_POWER, MOVE_ICE_BEAM, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_RAMPARDOS,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEAD_SMASH, MOVE_DOUBLE_EDGE, MOVE_THUNDER_PUNCH, MOVE_BRICK_BREAK},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne5[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_REGIROCK,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_SANDSTORM, MOVE_ICE_PUNCH, MOVE_THUNDERBOLT, MOVE_ROCK_SLIDE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_ROCK_SLIDE, MOVE_X_SCISSOR, MOVE_BRICK_BREAK, MOVE_SWORDS_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_TOXIC, MOVE_PROTECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_ROCK_SLIDE, MOVE_CRUNCH, MOVE_ICE_PUNCH, MOVE_DRAGON_DANCE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SCALD, MOVE_ANCIENT_POWER, MOVE_ICE_BEAM, MOVE_PROTECT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_HEAD_SMASH, MOVE_DRAGON_CLAW, MOVE_STEEL_WING, MOVE_DUAL_WINGBEAT},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly2[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB, MOVE_BULK_UP},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FOCUS_BLAST, MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_EXTREME_SPEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FAKE_OUT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_POLIWRATH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_HYPNOSIS},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly3[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB, MOVE_BULK_UP},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FOCUS_BLAST, MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_EXTREME_SPEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FAKE_OUT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_POLIWRATH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_HYPNOSIS},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly4[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB, MOVE_BULK_UP},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FOCUS_BLAST, MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_EXTREME_SPEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FAKE_OUT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_POLIWRATH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_HYPNOSIS},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly5[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB, MOVE_BULK_UP},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_SHADOW_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_FOCUS_BLAST, MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_EXTREME_SPEED},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_FAKE_OUT, MOVE_CLOSE_COMBAT, MOVE_ICE_PUNCH, MOVE_POISON_JAB},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_FLARE_BLITZ, MOVE_ROCK_SLIDE, MOVE_SHADOW_CLAW},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_POLIWRATH,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_WATERFALL, MOVE_ICE_PUNCH, MOVE_HYPNOSIS},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson2[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ROTOM_MOW,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_ENERGY_BALL, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ELECTIVIRE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson3[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ROTOM_MOW,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_ENERGY_BALL, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ELECTIVIRE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson4[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ROTOM_MOW,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_ENERGY_BALL, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ELECTIVIRE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_DRAGON_PULSE, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson5[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ZAPDOS,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_RAZOR_WIND, MOVE_THUNDERBOLT, MOVE_HEAT_WAVE, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ROTOM_MOW,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_ENERGY_BALL, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ELECTIVIRE,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_BRICK_BREAK, MOVE_ICE_PUNCH, MOVE_THUNDER_WAVE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_RAIKOU,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_THUNDERBOLT, MOVE_EXTREME_SPEED, MOVE_SHADOW_BALL, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_SUPERCELL_SLAM, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_THUNDER_WAVE},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery2[] = {
    {
    .iv = 255,
    .lvl = 38,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 36,
    .species = SPECIES_PONYTA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 38,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 40,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery3[] = {
    {
    .iv = 255,
    .lvl = 41,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_ROAR, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .species = SPECIES_PONYTA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery4[] = {
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_HOUNDOUR,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery5[] = {
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman2[] = {
    {
    .iv = 255,
    .lvl = 42,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 42,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman3[] = {
    {
    .iv = 255,
    .lvl = 47,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 47,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman4[] = {
    {
    .iv = 255,
    .lvl = 52,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 52,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman5[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TAKE_DOWN, MOVE_PROTECT, MOVE_FIRE_BLAST, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona2[] = {
    {
    .iv = 255,
    .lvl = 40,
    .species = SPECIES_DRATINI,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 38,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona3[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_HOOTHOOT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona4[] = {
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 49,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona5[] = {
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_THUNDERBOLT, MOVE_EARTHQUAKE, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_SKY_ATTACK, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza2[] = {
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 49,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_ANCIENT_POWER, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 49,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza3[] = {
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_DROWZEE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza4[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza5[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_SLOWKING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan2[] = {
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_POLIWAG,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_CRABHAMMER, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan3[] = {
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan4[] = {
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan5[] = {
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_HYDRO_PUMP, MOVE_PERISH_SONG}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_SHEER_COLD}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Angelo[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_ILLUMISE,
    .heldItem = ITEM_TWISTED_SPOON,
    .moves = {MOVE_PSYBEAM, MOVE_MOONLIGHT, MOVE_STRUGGLE_BUG, MOVE_SHOCK_WAVE}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_VOLBEAT,
    .heldItem = ITEM_MAGNET,
    .moves = {MOVE_SPARK, MOVE_MOONLIGHT, MOVE_POUNCE, MOVE_SHOCK_WAVE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darius[] = {
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_STARAPTOR,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Steven[] = {
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ROCK_SLIDE, MOVE_CRUNCH, MOVE_EARTHQUAKE, MOVE_THUNDER_PUNCH},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_WISE_GLASSES,
    .moves = {MOVE_SURF, MOVE_FLASH_CANNON, MOVE_ICE_BEAM, MOVE_EARTHQUAKE},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_TOXIC, MOVE_REST},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_X_SCISSOR, MOVE_ROCK_SLIDE, MOVE_BRICK_BREAK, MOVE_EARTHQUAKE},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_SCIZOR,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_BULLET_PUNCH, MOVE_POUNCE, MOVE_AERIAL_ACE, MOVE_ROCK_TOMB},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_METEOR_MASH, MOVE_PSYCHIC_FANGS, MOVE_ICE_PUNCH, MOVE_BULLET_PUNCH},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Anabel[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tucker[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Spenser[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Greta[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Noland[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lucy[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brandon[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres4[] = {
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory3[] = {
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin2[] = {
    {
    .iv = 110,
    .lvl = 35,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 110,
    .lvl = 35,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin3[] = {
    {
    .iv = 120,
    .lvl = 37,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 120,
    .lvl = 37,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 120,
    .lvl = 37,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin4[] = {
    {
    .iv = 130,
    .lvl = 39,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 130,
    .lvl = 39,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 100,
    .lvl = 39,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin5[] = {
    {
    .iv = 140,
    .lvl = 41,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 140,
    .lvl = 41,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 140,
    .lvl = 41,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando2[] = {
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer2[] = {
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_RHYDON,
    },
    {
    .iv = 200,
    .lvl = 45,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer3[] = {
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_STEELIX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer4[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_STEELIX,
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer5[] = {
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_GOLEM,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_RHYPERIOR,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_CLAYDOL,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_STEELIX,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_AERODACTYL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_LOTAD,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_SEEDOT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle4[] = {
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia4[] = {
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Mariela[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_CHIMECHO,
    .heldItem = ITEM_FOCUS_BAND,
    .moves = {MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_DAZZLING_GLEAM, MOVE_ENERGY_BALL},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_XATU,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_PSYCHIC, MOVE_AIR_SLASH, MOVE_SHADOW_BALL, MOVE_DAZZLING_GLEAM},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_BRONZONG,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_IRON_HEAD, MOVE_SHADOW_BALL, MOVE_ROCK_SLIDE},
    .abilityNums = 1
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alvaro[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MR_MIME,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_DAZZLING_GLEAM, MOVE_LIGHT_SCREEN, MOVE_REFLECT},
    .abilityNums = 1
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_EXEGGUTOR,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_REST},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_MUSCLE_BAND,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_FLARE_BLITZ, MOVE_SOLAR_BEAM, MOVE_SHADOW_BALL},
    .abilityNums = 0
    }
};

static const struct TrainerMonItemCustomMoves sParty_Everett[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_LIGHT_SCREEN, MOVE_REFLECT},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_BRIGHT_POWDER,
    .moves = {MOVE_PSYCHIC, MOVE_DARK_PULSE, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH},
    .abilityNums = 0
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_SLOWKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_HYDRO_PUMP, MOVE_SHADOW_BALL, MOVE_ICE_BEAM},
    .abilityNums = 1
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Red[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_CHARMANDER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leaf[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BULBASAUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_GROUDON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_KYOGRE,
    }
};
